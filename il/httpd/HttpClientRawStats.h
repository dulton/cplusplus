...
#ifndef _HTTP_CLIENT_BLOCK_RAW_STATS_H_
#define _HTTP_CLIENT_BLOCK_RAW_STATS_H_
...
class HttpClientRawStats : public L4L7_UTILS_NS::StatsTimerManager
{
  public:
    typedef ACE_Recursive_Thread_Mutex lock_t;

    HttpClientRawStats(uint32_t handle)
        : intendedLoad(0),
          activeConnections(0),
          mHandle(handle)
    {
...
#ifdef UNIT_TEST    
    HttpClientRawStats& operator=(const HttpClientRawStats& other)
    {
        ...
        minResponseTimePerUrlMsec = other.minResponseTimePerUrlMsec;
        maxResponseTimePerUrlMsec = other.maxResponseTimePerUrlMsec;
        avgResponseTimePerUrlMsec = other.avgResponseTimePerUrlMsec;
        sumResponseTimePerUrlMsec = other.sumResponseTimePerUrlMsec;
        rxResponseCode200 = other.rxResponseCode200;
    ...
    HttpClientRawStats(const HttpClientRawStats& other) { *this = other; }
#endif
    ...
    void Sync(bool zeroRates = false)
    {
        ...
        if (successfulTransactions)
            avgResponseTimePerUrlMsec = static_cast<uint64_t>(static_cast<double>(sumResponseTimePerUrlMsec) / successfulTransactions);
        else
            avgResponseTimePerUrlMsec = 0;
    }
    ...
    uint64_t minResponseTimePerUrlMsec;
    uint64_t maxResponseTimePerUrlMsec;
    uint64_t avgResponseTimePerUrlMsec;
    uint64_t sumResponseTimePerUrlMsec;
    
    uint64_t rxResponseCode200;       
  ...
  private:
    void ResetInternal(void)
    {
        ...
        minResponseTimePerUrlMsec = 0;
        maxResponseTimePerUrlMsec = 0;
        avgResponseTimePerUrlMsec = 0;
        sumResponseTimePerUrlMsec = 0;
