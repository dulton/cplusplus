/// @file
/// @brief Connection-oriented Datagram Socket (ACE) Handler implementation
///
...
CodgramSocketHandler::CodgramSocketHandler(uint32_t serial)
    : ACE_Svc_Handler<ACE_SOCK_CODgram, ACE_NULL_SYNCH>(0, 0, 0),
      mSerial(serial),
      mIsPending(false),
      mIsOpen(false),
...
CodgramSocketHandler::~CodgramSocketHandler()
{
...
int CodgramSocketHandler::open(void *factory)
{
...
int CodgramSocketHandler::handle_input(ACE_HANDLE fd)
{
...
int CodgramSocketHandler::handle_output(ACE_HANDLE fd)
{
...
int CodgramSocketHandler::handle_close(ACE_HANDLE fd, ACE_Reactor_Mask mask)
{
...
int CodgramSocketHandler::handle_timeout(const ACE_Time_Value& tv, const void *act)
{
...
void CodgramSocketHandler::Recv(std::string& buffer)
{
...
CodgramSocketHandler::messagePtr_t CodgramSocketHandler::Recv(void)
{
    messagePtr_t head;
...
bool CodgramSocketHandler::Send(const std::string& buffer)
{
    // SendData can complete a send, delegate to the engine, and close
    ACE_Event_Handler_var safeHandler(this);
    this->add_reference();
...
bool CodgramSocketHandler::Send(messagePtr_t mb)
{
    // SendData can complete a send, delegate to the engine, and close
    ACE_Event_Handler_var safeHandler(this);
    this->add_reference();
...
ssize_t CodgramSocketHandler::SendData(const void *buffer, size_t len)
{
    ...
    if (bytesSent < 0)
    {
        ...
        {
            TC_LOG_DEBUG(0, "Codgram socket handler " << this << " encountered error while sending: " << strerror(errno));
            return -1;
        }
    }
...
ssize_t CodgramSocketHandler::SendData(const iovec *iov, size_t len)
{
    const ssize_t bytesSent = this->peer().sendv(iov, (int)len);
...
ssize_t CodgramSocketHandler::ReceiveData(void *buffer, size_t len)
{
    ...
    if (bytesRecv == 0)
    {
        TC_LOG_DEBUG(0, "Codgram socket handler " << this << " closed");
        return -1;
    }
    else if (bytesRecv < 0)
    {
        ...
        {
            TC_LOG_DEBUG(0, "Codgram socket handler " << this << " encountered error while receiving: " << strerror(errno));
            return -1;
        }
    }
...
void CodgramSocketHandler::ScheduleTimer(IL_DAEMON_LIB_NS::TimerQueue::type_t type, IL_DAEMON_LIB_NS::TimerQueue::act_t *act, IL_DAEMON_LIB_NS::TimerQueue::delegate_t delegate, const ACE_Time_Value& delay)
{
    if (!mTimerQueue)
        return;
...
void CodgramSocketHandler::PurgeTimers(void)
{
    // Exclude other event handlers and timers
...
void CodgramSocketHandler::UpdateTxCountHandler(uint64_t sent)
{
