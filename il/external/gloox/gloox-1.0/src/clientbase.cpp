...
m_logInstance.err( LogAreaClassClientbase, "parser ending: \n" );
  }
...
//         etc... see gssapi-sasl-draft.txt
  ...
  void ClientBase::addIqID( const std::string& id, IqHandler* ih, int context, bool del )
  {
      TrackStruct track;
      track.ih = ih;
      track.context = context;
  ...
  void ClientBase::removeIqID( const std::string& id )
  {
      m_iqHandlerMapMutex.lock();
      IqTrackMap::iterator it_id = m_iqIDHandlers.find( id );
...
/*    if( m_connection )
    {
      delete m_connection;
    }*/    //taken out by spirent for cisco xmpp project
    m_connection = cb;
  }
  ...
  {
m_logInstance.err( LogAreaClassClientbase, "notify on tls connect was called, now calling on tls connect" );
...
//     util::ForEach( m_subscriptionHandlers, &SubscriptionHandler::handleSubscription, s10n );
  ...
  {
    m_iqHandlerMapMutex.lock();
    IqTrackMap tempMap = m_iqIDHandlers;
    IqTrackMap::iterator it_id_temp = tempMap.find( iq.id() );
    ...
    if( it_id != m_iqIDHandlers.end() && iq.subtype() & ( IQ::Result | IQ::Error ) )
    {
      ...
      m_iqIDHandlers.erase( it_id );
      m_iqHandlerMapMutex.unlock();
      (*it_id_temp).second.ih->handleIqID( iq, (*it_id_temp).second.context );
      if( (*it_id_temp).second.del )
        delete (*it_id_temp).second.ih;
      return;
    }
