...
static bool gnutls_initialized=false;
...
namespace gloox
{
  ...
  void GnuTLSClient::getCertInfo()
  {
    ...
    if(certListSize > 0){
        ...
        for( unsigned int i=0; !error && ( i<certListSize ) && &certList[i]; ++i )
        {
        ...
        if(!error){
            ...
            if( cert[certListSize-1] && ( gnutls_x509_crt_check_issuer( cert[certListSize-1], cert[certListSize-1] ) > 0 ))
                certListSize--;
