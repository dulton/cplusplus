...
*/
    ...
    // ---- PEPManager::PubSubOwner ----
    PEPManager::PubSubOwner::PubSubOwner( TrackContext context )
      : StanzaExtension( ExtPubSubOwner ), m_ctx( context ), m_form( 0 )
    {
    }

    PEPManager::PubSubOwner::PubSubOwner( const Tag* tag )
    ...
    PEPManager::PubSubOwner::~PubSubOwner()
    {
      delete m_form;
    }

    const std::string& PEPManager::PubSubOwner::filterString() const
    {
      static const std::string filter = "/iq/pubsub[@xmlns='" + XMLNS_PUBSUB_OWNER + "']";
      return filter;
    }

    Tag* PEPManager::PubSubOwner::tag() const
    {
    ...
    // ---- PEPManager::PubSub ----
    PEPManager::PubSub::PubSub( TrackContext context )
    ...
    PEPManager::PubSub::PubSub( const Tag* tag )
    ...
    PEPManager::PubSub::~PubSub()
    {
      delete m_options.df;
      util::clearList( m_items );
    }

    const std::string& PEPManager::PubSub::filterString() const
    {
      static const std::string filter = "/iq/pubsub[@xmlns='" + XMLNS_PUBSUB + "']";
      return filter;
    }

    Tag* PEPManager::PubSub::tag() const
    {
      if( m_ctx == InvalidContext )
        return 0;
      ...
      else if( m_ctx == PublishPEPItem )
      {
        Tag* p = new Tag( t, "publish" );
        p->addAttribute( "node", m_node );
      //  Tag* itemsTag = new Tag( p, "items");
        ItemList::const_iterator it = m_items.begin();
        for( ; it != m_items.end(); ++it )
          p->addChild( (*it)->tag() );
        if( m_options.df )
    ...
    StanzaExtension* PEPManager::PubSub::clone() const
    {
      PubSub* p = new PubSub();
    ...
    // ---- PEPManager ----
    PEPManager::PEPManager( ClientBase* parent )
      : m_parent( parent )
    {
      if( m_parent )
      {
    ...
    const std::string PEPManager::getSubscriptionsOrAffiliations( const JID& service,
    ...
    const std::string PEPManager::subscribe( const JID& service,
    ...
    const std::string PEPManager::unsubscribe( const JID& service,
                                            const std::string& node,
    ...
    const std::string PEPManager::subscriptionOptions( TrackContext context,
                                                    const JID& service,
                                                    const JID& jid,
                                                    const std::string& node,
                                                    ResultHandler* handler,
    ...
    const std::string PEPManager::requestItems( const JID& service,
                                             const std::string& node,
                                             const std::string& subid,
                                             int maxItems,
                                             ResultHandler* handler )
    ...
    const std::string PEPManager::publishItem( const JID& service,
                                            const std::string& node,
                                            ItemList& items,
                                            DataForm* options,
                                            ResultHandler* handler )
    ...
    const std::string PEPManager::publishPEPItem( const std::string& node,
                                            ItemList& items,
                                            DataForm* options,
                                            ResultHandler* handler )
    {
      ...
      //empty JID for service
      JID service;

      const std::string& id = m_parent->getID();
      IQ iq( IQ::Set, service, id );
      PubSub* ps = new PubSub( PublishPEPItem );
      ps->setNode( node );
      ps->setItems( items );
      ps->setOptions( EmptyString, options );
      iq.addExtension( ps );
      ...
      m_PEPtrackMapMutex.lock();
      m_PEPTrackMap[id] = "";  //FIXME find a string to use to search
      m_PEPtrackMapMutex.unlock();

      m_parent->send( iq, this, PublishPEPItem );
      return id;
    }


    const std::string PEPManager::deleteItem( const JID& service,
                                           const std::string& node,
                                           const ItemList& items,
                                           bool notify,
                                           ResultHandler* handler )
    ...
    const std::string PEPManager::createNode( const JID& service,
                                           const std::string& node,
                                           DataForm* config,
                                           ResultHandler* handler )
    {
    ...
    const std::string PEPManager::deleteNode( const JID& service,
                                           const std::string& node,
                                           ResultHandler* handler )
    {
      if( !m_parent || !handler || !service || node.empty() )
    ...
    const std::string PEPManager::getDefaultNodeConfig( const JID& service,
                                                     NodeType type,
                                                     ResultHandler* handler )
    {
      if( !m_parent || !handler || !service )
    ...
    const std::string PEPManager::nodeConfig( const JID& service,
                                           const std::string& node,
                                           DataForm* config,
                                           ResultHandler* handler )
    {
    ...
    const std::string PEPManager::subscriberList( TrackContext ctx,
                                               const JID& service,
                                               const std::string& node,
    ...
    const std::string PEPManager::affiliateList( TrackContext ctx,
                                              const JID& service,
                                              const std::string& node,
    ...
    const std::string PEPManager::purgeNode( const JID& service,
                                          const std::string&  node,
                                          ResultHandler* handler  )
    {
      if( !m_parent || !handler || !service || node.empty() )
    ...
    bool PEPManager::removeID( const std::string& id )
    {
      m_trackMapMutex.lock();
    ...
    void PEPManager::removeIqTrack( const std::string& id )
    {
        m_parent->removeIqID(id);
        m_trackMapMutex.lock();
        ResultHandlerTrackMap::iterator ith = m_resultHandlerTrackMap.find( id );
        if( ith == m_resultHandlerTrackMap.end() )
        {
    ...
    void PEPManager::handleIqID( const IQ& iq, int context )
    {
        ...
        case IQ::Result:
        {
          ...
          switch( context )
          {
            ...
            case PublishPEPItem:
            {
              ...
              {
                //add back to track maps so error will be detected
                m_parent->addIqID( id, this, PublishPEPItem, false);
                m_trackMapMutex.lock();
                m_resultHandlerTrackMap[id] = rh;
                m_trackMapMutex.unlock();
              }
              break;
            }
