                                       ...
                                       or NULL */
{
    ...
    if (RV_SDP_OBJ_IS_MESSAGE2(obj))
    {
        ...
        RvSdpMsg *msg;
        msg = ((RvSdpMsg*)obj);
        n = (prtclId)? (int)strlen(prtclId):0;
        n += (keyData)?(int)strlen(keyData):0;
...
RvSize_t rvSdpKeyMgmtDecodeKeyData(RvSdpKeyMgmtAttr* keyMgmt, unsigned char* decodedData, int dataLen)
{
    ...
    return rvDecodeB64((unsigned char*)keyMgmt->iKeyData,(RvInt)strlen(keyMgmt->iKeyData),decodedData,dataLen);
}
