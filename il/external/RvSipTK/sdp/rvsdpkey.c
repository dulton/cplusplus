...
{
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
    if (RV_SDP_BAD_SYNTAX_PARAM(k1->iBadSyntaxField) && 
        RV_SDP_BAD_SYNTAX_PARAM(k2->iBadSyntaxField))
    {
        if (0 != strcmp(k1->iBadSyntaxField, k2->iBadSyntaxField))
            return RV_FALSE;
        return RV_TRUE;
    }
    else
    ...
    if (0 != strcmp(k1->iTypeStr, k2->iTypeStr))
        return RV_FALSE;
    if (0 != strcmp(k1->iData, k2->iData))
        return RV_FALSE;
