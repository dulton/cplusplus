...
/*  MID PARSING FUNC
 *	Special attribute  parse function for the mid (media Id) special attribute.
 *  The constructed object is added to media descriptor.
 *
 *  If parsing or construction fails the correspondent status is returned.
 */
RvSdpSpecAttrParseSts rvSdpMidSpecialAttrParsing(
				const RvSdpSpecAttributeData* sad,                /* the special attribute data */
                RvBool                        createAsBadSyntax,  /* not used */
                RvSdpParserData*              pD,  /* the parser data instance */
                RvSdpCommonFields            *cm,  /* the current context common fields instance,
                ...
                REPLACING_DECL)                    /* used for zero-substitutions in input buffer */
{
    ...
    if((val == NULL) || *val == 0)
    {
        RV_SDP_WARNING_SET_AND_CREATE_WARNING(RvSdpWarnMidNoValueForMidAttr,pD);
        return rvSdpSpAttrPrsCrtRegular;
    }
    /*skip the spaces at the begining.if null is returned - there is no value*/
    rv = rvSdpParseSpace(&val);


    /*movingPtr points to the start of the value str now*/
    movingPtr=val;

    /*movingPtr will skip the txt chars till it finds space char.
    So now it points to the end of the midId value str*/
    rv = rvSdpParseNonSpace(&movingPtr,0);
    pp=movingPtr;
    /*if there is a non-space char after this position-there is
    more then one midId param in this attribute --> create generalAttr instead
    of specialAttr -because it is illegal to have more then 1 midId*/
    if(rvSdpParseSpace(&movingPtr))
    {
        RV_SDP_WARNING_SET_AND_CREATE_WARNING(RvSdpWarnMidIllegalMoreThenOneParam,pD);
        return rvSdpSpAttrPrsCrtRegular;
    }

    /*mark the end of the midId tag*/
    REPLACING2(pp);

    if (!rvSdpIsInteger(val))
    {
        /*if the mid tag is not integer- throw warning but create it as special mid attr*/
        RV_SDP_WARNING_SET_AND_CREATE_WARNING(RvSdpWarnMidIllegalMidValue,pD);
    }

    /*if the val is Int but the set operation has failed*/
    rv = rvSdpMediaDescrSetMidValueStr(pD->iMediaDescr,val);
    if( rv != RV_SDPSTATUS_OK)
        return rvSdpSpAttrPrsAllocFail;
    return rvSdpSpAttrPrsOK;
					...
					NULL,
						...
						-1,
							...
							NULL,
								rvSdpSpecialAttrParsing,
									RV_TRUE},
#endif /* #ifdef RV_SDP_FMTP_ATTR*/


/*media group & mid attr*/
#ifdef RV_SDP_MEDIA_GROUPING_ATTR
/*RV_SDP_MID_ATTR*/
    {"mid",
...
#endif
                        NULL,
                           rvSdpMidSpecialAttrParsing,
                             RV_FALSE},

/*RV_SDP_GROUP_ATTR*/
    {"group",
		SDP_FIELDTYPE_MEDIA_GROUP,
			(rvAttrDestructCB)rvSdpMediaGroupAttrDestruct,
				(rvAttrCopyCB)rvSdpMediaGroupAttrCopy2,
					(rvAttrGetValueCB)rvSdpMediaGroupAttrGetValue,
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
						RV_OFFSETOF(RvSdpMediaGroupAttr,iBadSyntaxField),
#endif
							(rvAttrReshuffleCB)rvSdpMediaGroupAttrReshuffle,
...
/*preconditions*/
    ...
    {"curr",
        SDP_FIELDTYPE_PRECONDITION,
            (rvAttrDestructCB)rvSdpPreconditionAttrDestruct,
                (rvAttrCopyCB)rvSdpPreconditionAttrCopy2,
                    (rvAttrGetValueCB)rvSdpPreconditionAttrGetValue,
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
                       RV_OFFSETOF(RvSdpPreconditionAttr,iBadSyntaxField),
#endif
                         (rvAttrReshuffleCB)rvSdpPreconditionAttrReshuffle,
...
{"path",
    ...
    {"rtcp",
		SDP_FIELDTYPE_RTCP,
			(rvAttrDestructCB)rvSdpRtcpAttrDestruct,
				(rvAttrCopyCB)rvSdpRtcpAttrCopy2,
					(rvAttrGetValueCB)rvSdpRtcpAttrGetValue,
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
						-1,
#endif
							(rvAttrReshuffleCB)rvSdpRtcpAttrReshuffle,
 ...
 ***************************************************************************/
RvSdpStatus rvSdpMediaDescrAddSpecAttr(
                                          ...
                                          RvSdpFieldTypes  fType)
{
    ...
    /*find if this spec attr appears already in the given mediaDesc*/
    attr = rvSdpGetSpecialAttr(&descr->iCommonFields,0,fType);

    /*If there is such attribute but there is no val --> remove the attribute*/
    if ( (attr != NULL) && (val == NULL))
    {
        rvSdpRemoveSpecialAttr(&descr->iCommonFields,0,fType);
        return RV_SDPSTATUS_OK;
    }
    /*If there is an attribute with legal val --> set the value*/
    else if (attr != NULL)
    {
        return rvSdpSetTextField(&attr->iAttrValue,attr->iObj,val);
    }
    /*If there is no such attribute at all & val is legal --> create an attribute & set the value*/
    else if ((attr == NULL) && (val != NULL))
    {
        const RvSdpSpecAttributeData* specAttrData;
        /*find specAttr in order to get the attr name*/
        specAttrData = rvSdpFindSpecAttrDataByFieldType(fType);
        if (!specAttrData)
            return RV_SDPSTATUS_ILLEGAL_SET;

        attr = rvSdpAddAttr2(descr->iObj,&descr->iCommonFields,NULL,
                             specAttrData->iName,NULL);
        if (attr != NULL)
        {
            rvSdpSetTextField(&attr->iAttrValue,attr->iObj,val);
            attr->iSpecAttrData = specAttrData;
        }
        if (!attr)
            return RV_SDPSTATUS_ALLOCFAIL;
    }
                                            ...
                                            const RvInt      midIntVal)
{
    ...
    if(midIntVal >= 0)
    {
        sprintf(midValAsStr,"%d",midIntVal);
 ...
 ***************************************************************************/
RvSdpAttribute* rvSdpSpecialAttrSetValue(RvSdpMsg         * msg,
                                         RvSdpCommonFields* commF,
										 RvSdpFieldTypes specAttrType,
                                         const char*        value,
                                         RvBool             isMultiple)
            ...
            const RvSdpAttribute* a2)
{
    ...
    a1Val = rvSdpAttributeGetValue((RvSdpAttribute*)a1);
    a2Val = rvSdpAttributeGetValue((RvSdpAttribute*)a2);
 ...
 *      descr - a pointer to the RvSdpMediaDescr object.
 ***************************************************************************/
const char* rvSdpMediaDescrGetAcceptTypesVal(RvSdpMediaDescr* descr)
{
    RvSdpAttribute * attr;
    const char     * accTypesStr;

	attr = rvSdpGetSpecialAttr(&descr->iCommonFields,0,SDP_FIELDTYPE_MSRP_ACC_TYPES);

    /*if there is no Accept-Types attribute in the mediaDesc -return null*/
    accTypesStr = (attr) ? attr->iAttrValue : NULL;
    return RV_SDP_EMPTY_STRING(accTypesStr);
 ...
 *      descr - a pointer to the RvSdpMediaDescr object.
 ***************************************************************************/
const char* rvSdpMediaDescrGetAcceptWrappedTypesVal(RvSdpMediaDescr* descr)
{
    RvSdpAttribute * attr;
    const char     * accWrappedTypesStr;

	attr = rvSdpGetSpecialAttr(&descr->iCommonFields,0,SDP_FIELDTYPE_MSRP_ACC_WRAPPED_TYPES);

    /*if there is no Accept-Wrapped-Types attribute in the mediaDesc -return null*/
    accWrappedTypesStr = (attr) ? attr->iAttrValue : NULL;
    return RV_SDP_EMPTY_STRING(accWrappedTypesStr);
 ...
 *      descr - a pointer to the RvSdpMediaDescr object.
 ***************************************************************************/
const char* rvSdpMediaDescrGetMaxSizeVal(RvSdpMediaDescr* descr)
{
    RvSdpAttribute * attr;
    const char     * maxSizeStr;

	attr = rvSdpGetSpecialAttr(&descr->iCommonFields,0,SDP_FIELDTYPE_MSRP_MAX_SIZE);

    /*if there is no max-size attribute in the mediaDesc -return null*/
    maxSizeStr = (attr) ? attr->iAttrValue : NULL;
    return RV_SDP_EMPTY_STRING(maxSizeStr);
 ...
 *      descr - a pointer to the RvSdpMediaDescr object.
 ***************************************************************************/
const char* rvSdpMediaDescrGetPathVal(RvSdpMediaDescr* descr)
{
    RvSdpAttribute * attr;
    const char     * pathStr;

	attr = rvSdpGetSpecialAttr(&descr->iCommonFields,0,SDP_FIELDTYPE_MSRP_PATH);

    /*if there is no Path attribute in the mediaDesc return null*/
    pathStr = (attr) ? attr->iAttrValue : NULL;
    return RV_SDP_EMPTY_STRING(pathStr);
...
RvSdpStatus rvSdpMediaDescrSetMidValueStr(
                                         ...
                                         const char     * midValStr)
{
    return rvSdpMediaDescrAddSpecAttr(descr,midValStr,SDP_FIELDTYPE_MEDIA_ID);
}
#endif /* RV_SDP_MEDIA_GROUPING_ATTR */
...
RvSdpStatus rvSdpMediaDescrSetAcceptTypesVal(RvSdpMediaDescr* descr,
                                             const char     * val)
{
    return rvSdpMediaDescrAddSpecAttr(descr,val,SDP_FIELDTYPE_MSRP_ACC_TYPES);
}
...
RvSdpStatus rvSdpMediaDescrSetAcceptWrappedTypesVal(RvSdpMediaDescr* descr,
                                                        const char     * val)
{
    return rvSdpMediaDescrAddSpecAttr(descr,val,SDP_FIELDTYPE_MSRP_ACC_WRAPPED_TYPES);
}
...
RvSdpStatus rvSdpMediaDescrSetMaxSizeVal(RvSdpMediaDescr* descr,
                                             const char     * val)
{
    return rvSdpMediaDescrAddSpecAttr(descr,val,SDP_FIELDTYPE_MSRP_MAX_SIZE);
}
...
RvSdpStatus rvSdpMediaDescrSetPathVal(RvSdpMediaDescr* descr,
                                          const char     * val)
{
    return rvSdpMediaDescrAddSpecAttr(descr,val,SDP_FIELDTYPE_MSRP_PATH);
}
