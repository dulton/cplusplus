				 ...
				 RvSize_t index)
{
    ...
    precondAttr = rvSdpMediaDescrGetFirstPrecondition(descr,precondName,&iter);

    for ( ;precondAttr;precondAttr=rvSdpMediaDescrGetNextPrecondition(precondName,&iter))
    {
		if (index == 0)
		{
...
RVSDPCOREAPI void rvSdpMediaDescrClearPrecondition(
					 ...
					 RvSdpPrecondName precondName)
{
	if (precondName == RV_SDP_PRECOND_NAME_ALL)
		rvSdpClearSpecialAttr(&descr->iCommonFields,SDP_FIELDTYPE_PRECONDITION);
	else
	{
		RvSdpListIter            iter;
		while (rvSdpMediaDescrGetFirstPrecondition(descr,precondName,&iter))
		{
			rvSdpListRemoveCurrent(&iter);
		}
...
RvSize_t rvSdpMediaDescrGetNumOfPrecondition(
                                       ...
                                       RvSdpListIter            * _doNotUseIt)
{
    RvSdpPreconditionAttr* precondAttr;
    RvSize_t               cnt=0;
    RvSdpListIter            iter;
    /*char                   precondName[RV_SDP_PRECOND_MAX_PRECOND_NAME] = {'\0'};*/
    RV_UNUSED_ARG(_doNotUseIt);


    if (ePrecondName == RV_SDP_PRECOND_NAME_ALL)
    {
        return rvSdpGetNumOfSpecialAttr(&descr->iCommonFields,SDP_FIELDTYPE_PRECONDITION);
    }
    else
    {
        /*convert the requested precondition name to string*/
        /*strcpy(precondName,rvSdpPreconditionNameVal2Txt(ePrecondName));*/

        /*find the first preconditionAttr in this mediaDescr*/
        precondAttr = rvSdpMediaDescrGetFirstPrecondition(descr,
                                                          ePrecondName,
                                                          &iter);

        /*go over the preconditionAttr list and count how many
          precond-name there are of each type */
        for ( ;precondAttr;precondAttr=(RvSdpPreconditionAttr*)rvSdpGetNextSpecialAttr(
                                                                          &iter,
                                                                          SDP_FIELDTYPE_PRECONDITION))
        {
           /* if (strcmp(precondName,rvSdpAttributeGetName(&precondAttr->iPreconditionAttribute)) == 0)*/
            if(ePrecondName == precondAttr->iePrecondName)
			{
                cnt++;
            }
        }
 ...
 *
 * Return Value:
 *      Pointer to the RvSdpPreconditionAttr  object or the NULL pointer if there is no
 *      precondition attriubute defined in the media descriptor.
 * ------------------------------------------------------------------------
 * Arguments:
 *      descr - a pointer to the RvSdpMediaDescr object.
                                                ...
                                                RvSdpListIter  * iter)
{
    ...
    /*find the first preconditionAttr in this mediaDescr*/
    precondAttr = (RvSdpPreconditionAttr*)rvSdpGetFirstSpecialAttr(&descr->iCommonFields,
                                                                   iter,
                                                                   SDP_FIELDTYPE_PRECONDITION);
    if (ePrecondName == RV_SDP_PRECOND_NAME_ALL || !precondAttr )
        return precondAttr;

    /*convert the requested precondition name to string*/
    /*strcpy(precondName,rvSdpPreconditionNameVal2Txt(ePrecondName));*/

    /*go over the preconditionAttr list and find the attr with the exact
    precond-name ("curr"\"des"\"conf")*/
    for ( ;precondAttr; precondAttr=(RvSdpPreconditionAttr*)rvSdpGetNextSpecialAttr(
                                                                   iter,
                                                                   SDP_FIELDTYPE_PRECONDITION))
    {
    ...
    /*such precondition name was not found in the preconditionAttr list*/
    return NULL;
}
 ...
 * ------------------------------------------------------------------------
 * General:
 *      Returns the next precondition attribute defined in the media descriptor that
 *      has the same precondition-name as required.
                                             ...
                                             RvSdpListIter*   iter)
{
    ...
    /*find the first preconditionAttr in this mediaDescr*/
    precondAttr = (RvSdpPreconditionAttr*)rvSdpGetNextSpecialAttr(iter,SDP_FIELDTYPE_PRECONDITION);
    if (ePrecondName == RV_SDP_PRECOND_NAME_ALL || !precondAttr)
        return precondAttr;
    ...
    /*go over the preconditionAttr list and find the attr with the same precond-name ("curr"\"des"\"conf")*/
    for (;precondAttr;
         precondAttr=(RvSdpPreconditionAttr*)rvSdpGetNextSpecialAttr(iter,SDP_FIELDTYPE_PRECONDITION))
    {
        /*if (strcmp(precondName,rvSdpAttributeGetName(&precondAttr->iPreconditionAttribute)) == 0)*/
 ...
 *      The index refers to the whole list of the precondition(regardless of name).
 *
 * Return Value:
                    ...
                    RvSize_t               index)
{
    RvSdpPreconditionAttr* precondAttr;
    /*char           precondName[RV_SDP_PRECOND_MAX_PRECOND_NAME] = {'\0'};*/
    RvSize_t       indexCounter = 0;
    RvSdpListIter  iter;

    precondAttr = rvSdpMediaDescrGetFirstPrecondition(
                                             descr,
                                             ePrecondName,
                                             &iter);
    ...
    if(precondAttr)
    {
        ...
        /*else*/
        /*go over the preconditionAttr list and find the attr
          with the same precond-name ("curr"\"des"\"conf")*/
        for (;precondAttr;
              precondAttr = rvSdpMediaDescrGetNextPrecondition(ePrecondName,&iter))
        {
            if ((precondAttr) && (ePrecondName == precondAttr->iePrecondName))
                /*(strcmp(precondName,rvSdpAttributeGetName(&precondAttr->iPreconditionAttribute)) == 0) )*/
            {
...
RVSDPCOREAPI RvSdpStatus rvSdpPreconditionAttrSetStrengthTag(
                                            RvSdpPreconditionAttr *    precondAttr,
                                            RvSdpPreconditionStrengthTag  eStrengthStr)
{
    return rvSdpPreconditionAttrSetStrengthTagStr(
                                         precondAttr,
...
RVSDPCOREAPI RvSdpPreconditionAttr* rvSdpMediaDescrAddPrecondition(
                                                 ...
                                                 const char*      directionStr)
{
    return rvSdpAddPreconditionAttr2(descr,&descr->iCommonFields,
                                      rvSdpPreconditionNameVal2Txt(precondName),
                                      typeStr,statusStr,strengthStr,directionStr,NULL);
}
...
RvSdpPreconditionAttr* rvSdpMediaDescrAddBadSyntaxPrecondition(RvSdpMediaDescr* descr, 
                                                               const char     * badSyn)
{
    return rvSdpAddPreconditionAttr2( descr,
                              &descr->iCommonFields,\
