 ...
 ***************************************************************************/
RvSdpMediaDescr* rvSdpMediaDescrConstructEx(RvSdpMsg*        msg,
                                            ...
                                            RvBool           dontConstruct)
{
    ...
    /*insert the media object to the nedia lineObj list*/
...
RvSdpMediaDescr* rvSdpMediaDescrFill(RvSdpMediaDescr* dest, const RvSdpMediaDescr* src)
{
    ...
    rvSdpMediaDescrClearVcId(dest);
    for (cnt = 0; cnt < src->iMediaVcIdsNum; cnt++)
        if (rvSdpMediaDescrAddVcId(dest,src->iMediaVcIdTypes[cnt],src->iMediaVcIdValues[cnt]) != RV_SDPSTATUS_OK)
            return NULL;

    if (rvSdpSetTextField(&dest->iMediaNASCtrlMethod,dest->iObj,src->iMediaNASCtrlMethod) != RV_SDPSTATUS_OK)
        return NULL;
                    ...
                    const RvSdpMediaDescr* descr2)
{
    ...
    if (descr1->iPort != descr2->iPort)
        return RV_FALSE;

    if (descr1->iNumOfPorts != descr2->iNumOfPorts)
        return RV_FALSE;

    if (0 != strcmp(descr1->iMediaTypeStr, descr2->iMediaTypeStr))
        return RV_FALSE;

    if (0 != strcmp(descr1->iProtocolStr, descr2->iProtocolStr))
        return RV_FALSE;

    if (descr1->iMediaFormatsNum != descr2->iMediaFormatsNum)
        return RV_FALSE;

    for (cnt = 0; cnt < descr1->iMediaFormatsNum; cnt++)
        if (0 != strcmp(descr1->iMediaFormats[cnt],descr2->iMediaFormats[cnt]))
            return RV_FALSE;

#ifdef RV_SDP_MEGACO_ENABLED
    if (descr1->iMediaVcIdsNum != descr2->iMediaVcIdsNum)
        return RV_FALSE;

    for (cnt = 0; cnt < descr1->iMediaVcIdsNum; cnt++)
    {
        if (descr1->iMediaVcIdTypes[cnt] != descr2->iMediaVcIdTypes[cnt])
            return RV_FALSE;

        if (0 != strcmp(descr1->iMediaVcIdValues[cnt], descr2->iMediaVcIdValues[cnt]))
            return RV_FALSE;

        if (0 != strcmp(descr1->iMediaNASCtrlMethod, descr2->iMediaNASCtrlMethod))
            return RV_FALSE;
    }
#endif /*RV_SDP_MEGACO_ENABLED*/
                                      ...
                                      void* obj, RvBool dontConstruct, RvBool mediaOnly)
{
    ...
    if (!mediaOnly)
    {
        /* this is probably case when the MediaDescr is created
           not in the context of external SdpMsg*/
        if (!obj)
            obj = dest->iObsMsg;
...
#ifdef RV_SDP_MEGACO_ENABLED
    rvSdpMediaDescrClearVcId(descr);
    rvSdpUnsetTextField(&descr->iMediaNASCtrlMethod,descr->iObj);
#endif /*RV_SDP_MEGACO_ENABLED*/
...
RVSDPCOREAPI RvBool rvSdpMediaDescrCompare(
                    ...
                    const RvSdpMediaDescr* descr2)
{
    RvBool bCommonFieldsMatch;
    RvBool bNotCommonFieldsMatch;

    bNotCommonFieldsMatch = rvSdpMediaDescrCompareNotCommonFileds(descr1, descr2);

    bCommonFieldsMatch = rvSdpCommonFieldCompare(&descr1->iCommonFields, &descr2->iCommonFields);

    return (bNotCommonFieldsMatch && bCommonFieldsMatch);
}
 ...
 ***************************************************************************/
const char*  rvSdpMediaDescrGetCodecNameByPayload(const RvSdpMediaDescr* descr,
                                                  int                    payload)
{
#define  ENC_MAX_NAME_LEN 15
    int            numOfRtpInMedia=0,index=0,rtpPayload= -1;
    RvSdpRtpMap*    rtpMapAttr;
    char*          encriptionName[ENC_MAX_NAME_LEN] = {""};
    const char* encrNamePtr = encriptionName[0];

    numOfRtpInMedia = (int)rvSdpMediaDescrGetNumOfRtpMap(descr);
    while (index < numOfRtpInMedia)
    {
        /*retrieve the rtp attribute*/
       rtpMapAttr = rvSdpMediaDescrGetRtpMap(descr,index);
       if (rtpMapAttr)
       {
           rtpPayload = rvSdpRtpMapGetPayload(rtpMapAttr);
           if (rtpPayload == payload)
           {
               encrNamePtr = rvSdpRtpMapGetEncodingName(rtpMapAttr);
               break;
           }
           index++;
       }
    ...
    return RV_SDP_EMPTY_STRING(encrNamePtr);
}
 ...
 *      descr - a pointer to valid RvSdpMediaDescr object.
 *      mediaType - the type of media.
 *      port - the media's port rvSdpMediaDescrConstruct.
 *      protocol - the media's protocol.
 *      badSyn - the proprietary formatted media field or NULL if standard media is 
 *               constructed.
 *      a  - allocator to be used for memory allocations, if allocator is NULL
...
RvSdpMediaDescr* rvSdpMediaDescrConstructA(RvSdpMediaDescr* descr, 
                                           RvSdpMediaType mediaType, RvInt32 port,
 ...
 *      descr - a pointer to the RvSdpMediaDescr object.
 ***************************************************************************/
RvInt32 rvSdpMediaDescrGetPort(const RvSdpMediaDescr* descr)
{
    return descr->iPort;
}
 ...
 ***************************************************************************/
void rvSdpMediaDescrSetPort(RvSdpMediaDescr* descr, RvInt32 port)
{
    descr->iPort = port;
}
