...
{
    RvSdpListIter l1iter, l2iter;
    void *l1Node, *l2Node;

    l1Node = rvSdpListGetFirst(l1,&l1iter);
    l2Node = rvSdpListGetFirst(l2,&l2iter);

    while (NULL != l1Node && NULL != l2Node)
    {
        if (RV_FALSE == (cf)(l1Node,l2Node))
            return RV_FALSE;

        l1Node = rvSdpListGetNext(&l1iter);
        l2Node = rvSdpListGetNext(&l2iter);
    }

    if (NULL != l1Node || NULL != l2Node)
        return RV_FALSE;
        ...
        rvListNodeDestructFunc destrF)      /* called for each unused pool object when the pool is destroyed */
{
    ...
    rvSdpListInitialize(&pool->iFreeObjects,(RvUint32)poolObjOffset,destrF);
...
void rvSdpPoolReturn(RvSdpObjPool* pool, void* p)
{
    int diff;
    rvSdpListHeadAdd(&pool->iFreeObjects,p);
    diff = (int)(pool->iFreeObjects.iListSize - pool->iInitSz);
