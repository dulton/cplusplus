...
RVSDPCOREAPI RvSdpMediaDescr* rvSdpMsgGetMediaDescrByGroup(
                                 ...
                                 RvUint               index)
{
    const char       midStrInGroupAttr[5] = {'\0'};
    const char*      p , *pp;
    RvSdpMediaDescr* medDesc;
    RvSdpListIter    iter1;


    p = midStrInGroupAttr;
    /*take the media-Id str from the midId parameter list in group attribute */
    p = rvSdpMediaGroupAttrGetMid(mediaGroupAttr,index);
    if (!p || !*p)
        return NULL;

    /* going over all media-desc should be done using "GetNext"
       because all the medias are kept in a linked-list */
    for (medDesc = rvSdpMsgGetFirstMediaDescr(msg,&iter1); medDesc; 
         medDesc = rvSdpMsgGetNextMediaDescr(&iter1))

    {
         /*find the mid val str in the current medDescr*/
         pp = rvSdpMediaDescrGetMidValueStr(medDesc);
             
         /*medDesc with the same midId str was found*/
         if (pp) 
         {
             if (strcmp(p,pp) == 0)
             return medDesc;
    ...
    /*medDesc with the same midId str (midStrInGroupAttr,as given in the media-group attr)
      was not found in the whole message*/
      return NULL;
...
void rvSdpMsgReshuffle(void* usrD, char** ptr)
{
    ...
    for (lo = msg->iHeadLO; lo; lo = lo->iNext)
    {
        ...
        case SDP_FIELDTYPE_MEDIA:
            ...
            for (cnt = 0; cnt < ((RvSdpMediaDescr*)p)->iMediaVcIdsNum; cnt++)
                rvSdpChangeText(ptr,&((RvSdpMediaDescr*)p)->iMediaVcIdValues[cnt]);
            rvSdpChangeText(ptr,&((RvSdpMediaDescr*)p)->iMediaNASCtrlMethod);
...
{
    return rvSdpMsgCopy2(dest,src,a,RV_FALSE,RV_FALSE);
}
...
RvSdpMsg* rvSdpMsgCopy2(RvSdpMsg* dest, const RvSdpMsg* src, RvAlloc* a, 
                        RvBool dontConstruct, RvBool sessDescrOnly)
{
    if (!dontConstruct)
    {
        ...
        rvSdpMsgDestructForReuse(dest, sessDescrOnly);
 ...
 ***************************************************************************/
RvBool rvSdpCommonFieldCompare(
            const struct _RvSdpCommonFields* commonFields1,
            const struct _RvSdpCommonFields* commonFields2)
{
    if (commonFields1->iInfo.iInfoTxt && commonFields2->iInfo.iInfoTxt)
    {
        if (0 != strcmp(commonFields1->iInfo.iInfoTxt, commonFields2->iInfo.iInfoTxt))
            return RV_FALSE;
    }
    else
    if (commonFields1->iInfo.iInfoTxt || commonFields2->iInfo.iInfoTxt)
    ...
    if (RV_FALSE == rvSdpListCompare(&commonFields1->iBandwidthList,
                                     &commonFields2->iBandwidthList,
                                     (rvSdpListCompareFunc)rvSdpBandwidthCompare))
        return RV_FALSE;

    if (RV_FALSE == rvSdpListCompare(&commonFields1->iConnectionList,
                                     &commonFields2->iConnectionList,
                                     (rvSdpListCompareFunc)rvSdpConnectionCompare))
        return RV_FALSE;

    if (RV_FALSE == rvSdpListCompare(&commonFields1->iAttrList,
                                     &commonFields2->iAttrList,
                                     (rvSdpListCompareFunc)rvSdpAttributeCompare))
        return RV_FALSE;

#if defined(RV_SDP_CHECK_BAD_SYNTAX)
    if (RV_FALSE == rvSdpListCompare(&commonFields1->iOtherList,
                                     &commonFields2->iOtherList,
                                     (rvSdpListCompareFunc)rvSdpOtherCompare))
        return RV_FALSE;
#endif

    if (RV_SDP_KEY_IS_USED(&commonFields1->iKey) && RV_SDP_KEY_IS_USED(&commonFields2->iKey))
    {
        if (RV_FALSE == rvSdpKeyCompare(&commonFields1->iKey,&commonFields2->iKey))
            return RV_FALSE;
    }
    else
    if (RV_SDP_KEY_IS_USED(&commonFields1->iKey) || RV_SDP_KEY_IS_USED(&commonFields2->iKey))
...
{
    rvSdpMsgDestructForReuse(msg, RV_FALSE/*sessDescrOnly*/);
...
RvSdpMediaDescr* rvSdpMsgAddMediaDescr(RvSdpMsg* msg, RvSdpMediaType media_type, 
                                       RvInt32 port, RvSdpProtocol protocol)
{
    RvSdpMediaDescr* md;
...
{
    RvSdpMsg* nmsg;
    nmsg = rvSdpMsgCopy2(NULL,msg,NULL,RV_FALSE,RV_FALSE);
    if (!nmsg)
        return RV_SDPSTATUS_ALLOCFAIL;
    rvSdpListTailAdd(&msgList->iList,(void*)nmsg);
    return RV_SDPSTATUS_OK;
}
 ...
 *      msg - pointer to constructed RvSdpMsg object.
 ***************************************************************************/
void rvSdpMsgListAppendMsg(RvSdpMsgList* msgList, const RvSdpMsg* msg)
{
    rvSdpListTailAdd(&msgList->iList,(void*)msg);
}
...
{
    return rvSdpMsgCopy2(dest,src,NULL,RV_FALSE,RV_FALSE);
}
...
{
    return rvSdpMsgCopy2(dest,src,NULL,RV_TRUE,RV_FALSE);
}
 ...
 * ------------------------------------------------------------------------
 * General:
 *      Copies the instance of Session Drescription part of RvSdpMsg from
 *      'src' to 'dest'. The Session Description part represents the RvSdpMsg
...
RVSDPCOREAPI RvSdpMsg* rvSdpMsgCopySessionDescription(
            ...
            const RvSdpMsg* src)
{
    return rvSdpMsgCopy2(dest,src,NULL,RV_TRUE,RV_TRUE);
}
...
* ------------------------------------------------------------------------
* General: 
*      Gets a MediaGroup attribute object by index. 
*          
* Return Value: 
*      The requested MediaGroup attribute object.
* ------------------------------------------------------------------------
* Arguments:
*      msg - a pointer to the RvSdpMsg object.
*      index - the index. The index should start at zero (0) and must be smaller 
...
***************************************************************************/
RvSize_t rvSdpMsgGetNumOfMediaGroup(const RvSdpMsg* msg)
{
    return  rvSdpGetNumOfSpecialAttr(&msg->iCommonFields,SDP_FIELDTYPE_MEDIA_GROUP);
...
*              in the list is retrieved by rvSdpMsgGetNumOfMediaGroup() call. 
***************************************************************************/
RvSdpMediaGroupAttr* rvSdpMsgGetMediaGroup(const RvSdpMsg* msg, 
                                           RvSize_t        index)
{
    return (RvSdpMediaGroupAttr*)rvSdpGetSpecialAttr(
                     (RvSdpCommonFields*)&msg->iCommonFields,
                      index,
 ...
 *             GetNext calls.
 ***************************************************************************/
RvSdpMediaGroupAttr* rvSdpMsgGetFirstMediaGroup(RvSdpMsg* msg, RvSdpListIter* iter)
{
    return (RvSdpMediaGroupAttr*)rvSdpGetFirstSpecialAttr(
                                 (RvSdpCommonFields*)&msg->iCommonFields,
                                  iter,
                                  SDP_FIELDTYPE_MEDIA_GROUP);
}
 ...
 *             to GetFirst/GetNext function.
 ***************************************************************************/
RvSdpMediaGroupAttr* rvSdpMsgGetNextMediaGroup(RvSdpListIter* iter)
{
    return (RvSdpMediaGroupAttr*)rvSdpGetNextSpecialAttr(iter,SDP_FIELDTYPE_MEDIA_GROUP);
}
 ...
 ***************************************************************************/
RvSdpMediaGroupAttr* rvSdpMsgAddBadSyntaxMediaGroup(RvSdpMsg* msg, const char* mediaGroup)
{
    return rvSdpAddMediaGroupAttr2(msg,&msg->iCommonFields,"",mediaGroup);
}
