                                     ...
                                     RvUint32     sizeOfCfg)
{
    ...
    if (__globals__ == NULL)
    {
        /* global data was not created before */
    ...
    __globals__->_sdpRefCount ++;

    if (__globals__->_sdpRefCount != 1)
        /* this is not the first attempt to initialize the SDP stack */
        return RV_OK;
...
/* SPIRENT_BEGIN */
            ...
            /* set default loglevel to except/error/warning only. */
            RvLogSetGlobalMask(g_sdpGlobal->pLogMgr, 
               RV_LOGLEVEL_EXCEP | RV_LOGLEVEL_ERROR  | RV_LOGLEVEL_WARNING);
#else
...
/*initialize the codecs if RV_SDP_CODECS_SUPPORTED is ON*/
#ifdef RV_SDP_CODECS_SUPPORTED
	RvLogInfo(&g_sdpGlobal->logSource,(&g_sdpGlobal->logSource,
		"		RV_SDP_CODECS_SUPPORTED is set"));
    crv = rvSdpCodecInitialize();
    if (crv != RV_OK)
    {
        RvLogError(&g_sdpGlobal->logSource,(&g_sdpGlobal->logSource,
            "		failed to initialize the codecs module"));
    }
#else
	{
        RvLogInfo(&g_sdpGlobal->logSource,(&g_sdpGlobal->logSource,
            "		RV_SDP_CODECS_SUPPORTED is not set (codecs module will not be used)"));
    }
...
* Output:  Log handle
***********************************************************************************/
RV_LOG_Handle RVCALLCONV RvSdpGetLogHandle()
{
    RV_SDP_USE_GLOBALS;
    if (g_sdpGlobal)
        return (RV_LOG_Handle) &g_sdpGlobal->logMgr;
    else
        return NULL;
}
