...
RvSdpStatus RvSdpParseFmtp(RvSdpCodecParamsInternal* prms, RvSdpCodecParserInternal *cp)
{
    ...
    for (;;)
    {
        ...
        if (isalpha((int)*name))
        {
            fmtpData = cp->iFmtpParamData[tolower((int)*name)-'a'];
...
RvSdpStatus RvSdpCodecModifyFmtpParam(
    ...
    const RvChar* name,
    const RvChar* value,     /* name & value to look for */
    RvBool isRemove,
    RvBool removeAttrIfLast, /* will remove the whole 'a=fmtp:' line if the last name=value pair is removed*/
    RvBool addIfNotExist,    /* will add the a=fmtp line in case it was not existing */
    RvBool isGet,
    RvInt* getIndex,         /* brings back the index of name=value pair */
    RvBool isCount,          /* only returns the numner of name=value pairs in 'count'*/
    RvInt *count,
    RvInt index,             /* looks nto by name in the 'name=value' pair but the pair with index */
    RvChar* valBuff,         /* used to copy the value, when getting */
    RvInt valBuffLen,        /* the length of 'valBuff' */
    RvUint32 payload,
    RvChar separator,
    RvAlloc* a)
{
...
#define BUFF_SZ 256
    ...
    if (attrP)
    {
        ...
        for (;;)
        {
            ...
            if (isCount)
                continue;
            if ((name && nLen == nameLen && RvStrncasecmp(name,n,nLen) == 0) || (!name && cnt-1 == index))
            {
                found = RV_TRUE;
                if (getIndex)
                    *getIndex = cnt-1;
    ...
    attrValLen = (attrVal) ? (int)strlen(attrVal):0;

    if (!isRemove)
    {
        newValLen = (value)?(int)strlen(value):0;
        newNameLen = (int)strlen(name);
...
RVSDPCOREAPI RvSdpStatus RvSdpCodecFmtpParamGetByName2(
    ...
    RvInt* index)
{
    RvSdpStatus st = RV_SDPSTATUS_NULL_PTR;

    for (; *separator; separator++)
    {
        st = RvSdpCodecModifyFmtpParam(descr,name,NULL,RV_FALSE,RV_FALSE,
            RV_FALSE,RV_TRUE,index,RV_FALSE,NULL,0,value,valLen,payload,*separator,NULL);
        if (st == RV_SDPSTATUS_OK)
            return st;
    }
    return st;
}
