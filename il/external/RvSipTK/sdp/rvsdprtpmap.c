                      ...
                      const char* badSyn, RvAlloc* a)
{
    ...
    {
        int n = 0;
        n += (badSyn) ? (int)strlen(badSyn) : 0;
        n += (encoding_name) ? (int)strlen(encoding_name) : 0;
...
char* rvSdpRtpMapGetValue(RvSdpRtpMap* rtpMap, char* txt)
{
    ...
    if (rtpMap->iPayload == RV_SDP_RTPMAP_FORMAT_ANY)
        *txt++ = '$';
    else 
#endif/*RV_SDP_MEGACO_EXTRA_ENABLED*/
    {
                ...
                REPLACING_DECL)         /* used to set zero bytes in the input buffer */
{
    ...
    *encParams = NULL;
    ...
    if (!rvSdpParseInt(0,0,(RvUint32*)payload,RV_TRUE,RV_FALSE,&ptr,&intW))
    {
        ...
        if (*ptr == '$') {
            ptr++;
            *payload = RV_SDP_RTPMAP_FORMAT_ANY;
        }
        else 
#endif /* */
        {
