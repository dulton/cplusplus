...
{
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
    if (RV_SDP_BAD_SYNTAX_PARAM(bw1->iBadSyntaxField) && 
        RV_SDP_BAD_SYNTAX_PARAM(bw2->iBadSyntaxField))
    {
        if (0 != strcmp(bw1->iBadSyntaxField, bw2->iBadSyntaxField))
            return RV_FALSE;
        return RV_TRUE;
    }
    else
    ...
    if (bw1->iBWValue != bw2->iBWValue)
        return RV_FALSE;
    if (0 != strcmp(bw1->iBWType, bw2->iBWType))
        return RV_FALSE;
