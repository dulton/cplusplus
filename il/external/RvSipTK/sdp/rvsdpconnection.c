                          ...
                          RvAlloc* a, RvBool dontConstruct)
{
    ...
    {
        int l = 0;
        l += (address) ? (int)strlen(address):0;
...
{
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
    if (RV_SDP_BAD_SYNTAX_PARAM(c1->iBadSyntaxField) && 
        RV_SDP_BAD_SYNTAX_PARAM(c2->iBadSyntaxField))
    {
        if (0 != strcmp(c1->iBadSyntaxField, c2->iBadSyntaxField))
            return RV_FALSE;
        return RV_TRUE;
    }
    else
    ...
    if (c1->iTtl != c2->iTtl)
        return RV_FALSE;
    if (c1->iNumAddr != c2->iNumAddr)
        return RV_FALSE;
    if (0 != strcmp(c1->iAddress, c2->iAddress))
        return RV_FALSE;
    if (0 != strcmp(c1->iNetTypeStr, c2->iNetTypeStr))
        return RV_FALSE;
    if (0 != strcmp(c1->iAddrTypeStr, c2->iAddrTypeStr))
        return RV_FALSE;
