...
#define RV_SDP_MEGACO_MANDATORY  RV_FALSE 
#else
#define RV_SDP_MEGACO_MANDATORY  RV_TRUE 
#endif
...
/* index    tag  mandatory  testMultipleFunc             valueParseFunc             allowSpaceValues*/
    ...
    { 1,    'o', RV_SDP_MEGACO_MANDATORY,   
                            rvSdpMsgOriginIsSet,         rvSdpParseOrigin,          RV_FALSE},
    { 2,    's', RV_SDP_MEGACO_MANDATORY,   
                            rvSdpMsgSessionNameIsSet,    rvSdpParseSessionName,     RV_TRUE},
    { 3,    'i', RV_FALSE,  rvSdpMsgInformationIsSet,    rvSdpParseInformation,     RV_TRUE},
    ...
    { 9,    't', RV_SDP_MEGACO_MANDATORY,   
                            NULL,                        rvSdpParseTime,            RV_FALSE},
...
{
/* index    tag  mandatory  testMultipleFunc                    valueParseFunc          allowSpaceValues*/
    { 0,    'i', RV_FALSE,  rvSdpMediaDescrInformationIsSet,    rvSdpParseInformation,  RV_TRUE},  
...
endOfBuffer:
    ...
    /* found tag; save it*/
    pD->iCurrTag = (RvChar) tolower((int)*p);
...
void rvSdpGetWarningText(RvSdpParserWarningData* wd, char *txt, int len)
{
    ...
    static const _rvSdpWarnText sWT[] =
    {
        ...
        {RvSdpWarnTooManyMediaFormats,"Too many media formats"},
...
#ifdef RV_SDP_MEGACO_ENABLED        
        {RvSdpWarnIllegalVcid,"The VC is illegal"},
        ...
        /*a=mid warnings*/
        {RvSdpWarnMidNoValueForMidAttr,"Mid Id attribute must have a value.Created as general attribute"},
        {RvSdpWarnMidIllegalMidValue,"Illegal mid value in Mid attribute.Should have Integer value."},
        {RvSdpWarnMidIllegalMoreThenOneParam,"Mid attribute can contain only one midId parameter.Created as general attribute"},
        
        /*a=group warnings*/
        {RvSdpWarnMediaGroupNoSemanticsVal,"Missing Semantics value in media-group attribute"},
        {RvSdpWarnMediaGroupIllegalMidIdVal,"Illegal mid value in media-group attribute .Should have Integer value"},
        {RvSdpWarnMediaGroupMidIdParamsTooMany,"Too many mid Id parameters for the media-group attribute"},
        {RvSdpWarnMediaGroupNoMidIdParams,"Missing Mid Id list in media-group attribute"},
        {RvSdpWarnMediaGroupIllegalSemantics,"Illegal Semantics value in media-group attribute"},
        
#endif /*RV_SDP_MEDIA_GROUPING_ATTR*/

        /*a=curr,a=des,a=conf warnings*/
#ifdef RV_SDP_PRECONDITIONS_ATTR
        {RvSdpWarnPrecondNumOfParametersIllegal,"Preconditions:Illegal number of parameters in attribute value"},
        {RvSdpWarnPrecondMissingOneParameter,"Preconditions: Missing one parameter in attribute value"},
		{RvSdpWarnPrecondNoParameters,"Preconditions: No parameters for precondition"},
		{RvSdpWarnPrecondTooManyParameters,"Preconditions : Too Many Parameters in attribute value"},
...
#define RV_SDP_STRICT_ADDRESS_PARSING 0
#if RV_SDP_STRICT_ADDRESS_PARSING
 ...
 */
RvBool rvSdpParseNetType_AddrType_Addr(char** ptr, char** netTypeTxt, char** addrTypeTxt, char** addr, 
...
/*unknown*/ {NULL,  NULL,   NULL,   NULL,       NULL,   NULL,   NULL,   NULL,   NULL,       NULL,   NULL,   NULL,},   /*unknown*/            
            ...
            if (prsAddr[netType][addrType])
        {
                if (!(*prsAddr[netType][addrType])(netType,addrType,*addr,w))
                    RV_SDP_WARNING_SET_AND_CREATE_WARNING(*w,pD);
            }        
        } 
    }
#endif    
                                       ...
                                       contradicts the standard */
{
    ...
    if (!rvSdpParseNetType_AddrType_Addr(&p,&netTypeTxt,&addrTypeTxt,
										 &addr,&naw,pD,REPLACING_ARGS))
    {
    ...
    if (rvSdpOriginConstruct2(pD->iMsg,&pD->iMsg->iOrigin,user,sessId,sessVer,
                            RV_SDPNETTYPE_UNKNOWN,RV_SDPADDRTYPE_UNKNOWN,addr,NULL,NULL,RV_FALSE) == NULL)
        goto allocFail;
                            ...
                            RvBool replaceExisting)
{
    ...
    if (!rvSdpParseNetType_AddrType_Addr(&p,&netTypeTxt,&addrTypeTxt,&addr,&naw,pD,REPLACING_ARGS))
    {
        pD->iWarning.iWarningNum = naw; 
        goto parseFailed;
    }
    ...
    conn = rvSdpAddConnection2(obj,RV_SDPNETTYPE_UNKNOWN,RV_SDPADDRTYPE_UNKNOWN,addr,NULL);
    if (conn == NULL)
        goto allocFail;
...
RvSdpMediaDescr* createMedia(RvSdpMsg *msg, int portNum, int subPortNum, char* mediaTxt, char* protoTxt
#ifdef RV_SDP_MEGACO_ENABLED                             
                             ,RvUint vcidNum, RvSdpVcId* vcidTypes, RvChar** vcidVals
#endif
                             )
{
    RvSdpMediaDescr* md;

    md = rvSdpMediaDescrConstructEx(msg,NULL,RV_SDPMEDIATYPE_UNKNOWN,
        portNum,RV_SDPPROTOCOL_UNKNOWN,
        NULL,NULL,RV_FALSE);
    if (md == NULL)
        return NULL;
...
#ifdef RV_SDP_MEGACO_ENABLED
    if (portNum == RV_SDP_PORT_ATM)
    {                
        RvUint i;
        for (i = 0; i < vcidNum; i ++)
        {
            if (rvSdpMediaDescrAddVcId(md,vcidTypes[i],vcidVals[i]) != RV_SDPSTATUS_OK)
                return NULL;
        }
    }
#endif /*RV_SDP_MEGACO_ENABLED*/    
...
{
    char *p, *pp, *mediaTxt, *protoTxt, *fmt = NULL, *nasCtrlMethod = NULL;
    RvSdpMediaDescr *md = NULL;
    RvInt32 portNum;
    RvUint32 unum,subPortNum = 0,cnt;
    RvBool  retV = RV_TRUE;   
    RvSdpIntegerParseWarning intW;
    RvSdpParseWarning w;
#ifdef RV_SDP_MEGACO_ENABLED
    RvChar* vcidVals[RV_SDP_MEDIA_VCID_MAX] = {NULL};
    RvSdpVcId vcidTypes[RV_SDP_MEDIA_VCID_MAX] = {RV_SDPVCID_UNKNOWN};
    RvSize_t vcidNum = 0;
#endif /*RV_SDP_MEGACO_ENABLED*/
...
#ifdef RV_SDP_MEGACO_ENABLED
    if (!strncmp(mediaTxt,"nas/",4))
        nasCtrlMethod = mediaTxt + 4;
#endif /*RV_SDP_MEGACO_ENABLED*/
    ...
    if (!rvSdpParseSpace(&p))
    {
        ...
        {
            /* possible m= line is
               m = nas/NAS_CTRL_METHOD [port[/subPort]]
            */
            /* this case is for m=nas/NAS_CTRL_METHOD */
            md = rvSdpMediaDescrConstructEx(pD->iMsg,NULL,RV_SDPMEDIATYPE_UNKNOWN,
                RV_SDP_PORT_NOTSET,RV_SDPPROTOCOL_UNKNOWN,
                NULL,NULL,RV_FALSE);
            if (md == NULL || rvSdpMediaDescrSetNASCtrlMethod(md,nasCtrlMethod) != RV_SDPSTATUS_OK)
                goto allocFail;  
...
#ifdef RV_SDP_MEGACO_ENABLED
    /* function rvSdpParseVcId will fail in two case:
        1. the input does not look like vcid definition
           in this case the 'w' stays with RvSdpWarnNoWarning value;
        2. the input does looks like vcid definition but we failed to parse it
           in this case 'w' will have some warning value and the media line parsing fails;
    */
    if (RV_SDP_IS_ALPHA(*p) && !nasCtrlMethod)
    {
        if (!rvSdpParseVcId(vcidVals,vcidTypes,&vcidNum,&p,&w,REPLACING_ARGS))
        {
            pD->iWarning.iWarningNum = (w==RvSdpWarnNoWarning)?RvSdpWarnIllegalMediaPort:w;
            goto parseFailed;
        }
        portNum = RV_SDP_PORT_ATM;
    }
    else 
#endif /*RV_SDP_MEGACO_ENABLED*/
    {
        ...
        {
            if (*p == '$')
                portNum = RV_SDP_PORT_ANY;
            else
                portNum = RV_SDP_PORT_IGNORE;
            p++;
        }    
        else if (!rvSdpParseInt(0,0,&unum,RV_TRUE,RV_FALSE,&p,&intW))
    {
        if (intW == RvSdpWarnNumberOverflow)
        {
    ...
    if (*p == '/')
    {
        ...
        if (nasCtrlMethod)
        {
            /* if the nas-control-method was set nothing can come after port/subport construction */
            if (*p)
            {
            ...
            md = rvSdpMediaDescrConstructEx(pD->iMsg,NULL,RV_SDPMEDIATYPE_UNKNOWN,
                portNum,RV_SDPPROTOCOL_UNKNOWN,NULL,NULL,RV_FALSE);
            if (md == NULL || rvSdpMediaDescrSetNASCtrlMethod(md,nasCtrlMethod) != RV_SDPSTATUS_OK)
                goto allocFail;  
            rvSdpMediaDescrSetNumOfPorts(md,subPortNum);   
            rvSdpListTailAdd(&pD->iMsg->iMediaDescriptors,md);
    ...
    for (cnt = 0;;cnt++)
    {
        if (*p == 0)
        {
            ...
            if (!md)
                md = createMedia(pD->iMsg,portNum,subPortNum,mediaTxt,protoTxt
#ifdef RV_SDP_MEGACO_ENABLED                
                ,vcidNum,vcidTypes,vcidVals
#endif
                );
            if (!md)
                goto allocFail;
            ...
            {
                pD->iWarning.iWarningNum = RvSdpWarnTooManyMediaFormats;
                goto parseFailed;
            }
...
#ifdef RV_SDP_MEGACO_EXTRA_ENABLED
            if (*fmt == '$')
            {
                p++;
            }
...
#endif
            /* if this is for MSRP the formats list may have '*' value */
            if (cnt == 0 && strcmp(fmt,"*") == 0)
                p ++;
            else
            {
                pD->iWarning.iWarningNum = RvSdpWarnIllegalMediaFormat;
