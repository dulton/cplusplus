...
{
    if (RV_SDP_OBJ_IS_MESSAGE(origin))
    {
        int l;
        l = (userName) ? (int)strlen(userName):1;
        l += (sessionId) ? (int)strlen(sessionId):1;
        l += (version) ? (int)strlen(version):1;
        l += (address) ? (int)strlen(address):1;
        l += (addrType) ? (int)strlen(addrType):1;
        l += (netType) ? (int)strlen(netType):1;
        rvSdpMsgPromiseBuffer((RvSdpMsg*)(origin->iObj),l);
    }
