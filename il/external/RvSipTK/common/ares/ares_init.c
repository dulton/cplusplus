                   ...
                   int tcp_bufflen, struct ares_options *options, int optmask)
{
    ...
    for (i=0; i < max_servers; ++i)
    {
        ...
        status = RvMemoryAlloc(NULL, tcp_bufflen,
                               channel->logMgr, (void**)&server->tcp_buffer);
        if (status != RV_OK)
            return ARES_ESERVICE;
    ...
    status = RvCondConstruct(&channel->inCallbackCond, channel->logMgr);
    if(status != RV_OK) {
        return status;
    }
...
void ares_destruct(RvDnsEngine *channel)
{
    ...
    /* ares_cancel_queries should be called in 'locked' state of resolver */   
    RvLockGet(&channel->lock, channel->logMgr);
   (void)ares_cancel_queries(channel, RV_FALSE);
    ...
    RvCondDestruct(&channel->inCallbackCond, channel->logMgr);
