...
// Really it's very dirty and bad idea to patch RadVision stack for use multi-threaded Select engines
// in sigle-threaded stack. But we have it already done.
static RvLock thread_lock; // Used for serialize access to engine_num (since select engines is used by concurrent threads)
static RvUint32 engine_num;// number of Select engines, associated with RvThreadCurrentPtr

void rvThreadCurrentLock(void)
{
    RV_CRITICAL_BEGIN();
}
void rvThreadCurrentUnLock(void)
{
    RV_CRITICAL_END();
}
RvUint32 __rvThreadCurrentHold(void)
{
    return engine_num++;
}
RvUint32 __rvThreadCurrentPut(void)
{
    return engine_num--;
}
#endif // UPDATED_BY SPIRENT
...
//SPIRENT_BEGIN
#if defined(UPDATED_BY_SPIRENT) && (RV_THREADNESS_TYPE == RV_THREADNESS_SINGLE)
/* TLS key for different select engines in the single threaded stack*/
static pthread_key_t RvSpirentSelectEngineTlsKey;
static volatile RvUint32 RvSpirentSelectEngineKeyIndex = RV_THREAD_MAX_VARS;
#endif
//SPIRENT_END
...
#if defined(UPDATED_BY_SPIRENT) && (RV_THREADNESS_TYPE == RV_THREADNESS_SINGLE)
            /* TLS key for different select engines in the single threaded stack*/
            if (name && (0 == strncmp("RvSelectEngine", name, RV_THREAD_MAX_VARNAMESIZE-1)))
            {
                if (pthread_key_create(&RvSpirentSelectEngineTlsKey, NULL))
                {
                    RvLockRelease(&RvThreadLock,logMgr);
                    RvThreadLogError((&logMgr->threadSource, "RvThreadCreateVar: Error while pthread_key_create()"));
                    return RvThreadErrorCode(RV_THREAD_ERROR_NOVARS);
                }
                RvSpirentSelectEngineKeyIndex = i;
            }
#endif
            //SPIRENT_END
...
#if defined(UPDATED_BY_SPIRENT) && (RV_THREADNESS_TYPE == RV_THREADNESS_SINGLE)
    /* TLS key for different select engines in the single threaded stack*/
    if ((RvSpirentSelectEngineKeyIndex < RV_THREAD_MAX_VARS)
        && (index == RvSpirentSelectEngineKeyIndex))
    {
        if (pthread_key_delete(RvSpirentSelectEngineTlsKey))
        {
            RvThreadLogError((&logMgr->threadSource, "RvThreadDeleteVar: Error while pthread_key_delete()"));
        }
        RvSpirentSelectEngineKeyIndex = RV_THREAD_MAX_VARS;
    }
#endif
    //SPIRENT_END
    RvLockRelease(&RvThreadLock,logMgr);
...
#if defined(UPDATED_BY_SPIRENT) && (RV_THREADNESS_TYPE == RV_THREADNESS_SINGLE)
    /* TLS key for different select engines in the single threaded stack*/
    if (RvSpirentSelectEngineKeyIndex == index) {
        if (pthread_setspecific(RvSpirentSelectEngineTlsKey, value)){
            RvThreadLogError((&logMgr->threadSource, "RvThreadSetVar: selectEnginekey not initialized"));
            return RV_ERROR_UNINITIALIZED; /* No TLS key */
        }
    } else
        th->vars[index] = value;
#else
    ...
    OUT void **value)
{
    ...
    /* TLS key for different select engines in the single threaded stack*/
    if (RvSpirentSelectEngineKeyIndex == index) {
        *value = pthread_getspecific(RvSpirentSelectEngineTlsKey);
    } else {
        th = RvThreadCurrent();
        if(th == NULL) {
