...
/* SPIRENT_BEGIN */
/* The following definition have been moved to rvsocket.h to make socketoption available for other modules! */
#if !defined(UPDATED_BY_SPIRENT)
...
/* SPIRENT_BEGIN */
            ...
            RvAddressConstructIpv6(address,
                (RvUint8*)(sin->sin6_addr.s6_addr), RvConvertNetworkToHost16(sin->sin6_port), (RvUint32)sin->sin6_scope_id, 0);
#else
            RvAddressConstructIpv6(address,
...
#if defined(UPDATED_BY_SPIRENT)
void Spirent_RvSocketBindDevice(
                        IN RvSocket*         sock,
                        IN const RvChar*    if_name,
                        IN RvLogMgr*         logMgr)
{
  RvSockLogEnter((&logMgr->socketSource,"Spirent_RvSocketBindDevice - Checking for socket HW binding....\n"));

  if(if_name && *if_name) {
    struct ifreq ifr;
    RvSockLogWarning((&logMgr->socketSource,"Spirent_RvSocketBindDevice - Found ARP mapping. Binding socket to interface %s", if_name));

    memset((char *)&ifr, 0, sizeof(ifr));
    strncpy(ifr.ifr_name, if_name, sizeof(ifr.ifr_name));
    if (RvSocketSetSockOpt(sock, SOL_SOCKET, SO_BINDTODEVICE /*| MSG_DONTROUTE*/, (char*)&ifr, sizeof(ifr)) != 0) {
      RvSockLogError((&logMgr->socketSource,"Spirent_RvSocketBindDevice - Failed to bind socket to interface %s", ifr.ifr_name));
    }
  }
}

RvStatus RvSocketAddressToSockAddrExternal(
    IN  RvAddress*      address,
    OUT RvUint8* sockaddr,
    OUT RvInt*            socklen) 
{
   return RvSocketAddressToSockAddr(address,(RV_SOCKADDR_PTR)sockaddr,socklen);
}

RvInt RvUdpSend(RvSocket sock,void *buf, RvInt len,RvInt flags, RvUint8* sockdata, RvInt socklen) 
{
   return UDP_SEND(sock,buf,len,flags,(RV_SOCKADDR_PTR)sockdata,socklen);
}
...
/* SPIRENT_BEGIN */
/*
 * COMMENTS:  This is the case that the fixed port number was requested.
 *              Allow reuse this port for binding.  
 */
#if defined(UPDATED_BY_SPIRENT)	
            if ( RvSocketReuseAddr(&ourSocket,logMgr ) != RV_OK)
            {
                    RvSockLogError((&logMgr->socketSource,
                        "RvSocketBind: Failed to setsockopt(SO_REUSEADDR) %d",
                        RvSocketErrNo));
            }
#endif /* defined(UPDATED_BY_SPIRENT) */ 
/* SPIRENT_END   */
    ...
    OUT RvAddress*   remoteAddress)
{
    ...
    *newSocket = WSAAccept(ourSocket, (RV_SOCKADDR_PTR)&sockdata[0], &socklen, NULL, 0);
    if ((*newSocket) == INVALID_SOCKET)
        ...
        //SPIRENT_BEGIN
#if defined(UPDATED_BY_SPIRENT)
        while((*newSocket = ACCEPT_SOCKET(ourSocket, (RV_SOCKADDR_PTR)sockdata, &socklen))==-1 && errno==EINTR);
        if(*newSocket<0)
#else
...
/* SPIRENT_BEGIN */
#if defined(UPDATED_BY_SPIRENT)
		RvSocketSetLinger(newSocket, 0, logMgr);
#endif /* defined(UPDATED_BY_SPIRENT) */ 
/* SPIRENT_END */
    ...
    OUT RvSize_t*   bytesSent)
{
    ...
    if (remoteAddress == NULL) {
       //SPIRENT_BEGIN
#if defined(UPDATED_BY_SPIRENT)
       while((errLen = TCP_SEND(ourSocket, (char *)buffer, (int)bytesToSend, RV_MSG_NOSIGNAL))==-1 && errno==EINTR);
#else
    ...
    else
    {
        ...
        //SPIRENT_BEGIN
#if defined(UPDATED_BY_SPIRENT)
        while((errLen = UDP_SEND(ourSocket, (char *)buffer, (int)bytesToSend, RV_MSG_NOSIGNAL, (RV_SOCKADDR_PTR)sockdata, socklen))==-1 && errno==EINTR);
#else
    ...
    OUT RvAddress*  remoteAddress)
{
    ...
    if (isTcpSocket)
       //SPIRENT_BEGIN
#if defined(UPDATED_BY_SPIRENT)
       while((errLen = TCP_RECV(ourSocket, (char *)buffer, (int)bytesToReceive, 0))==-1 && errno==EINTR);
#else
    ...
    else
    {
       ...
       //SPIRENT_BEGIN
#if defined(UPDATED_BY_SPIRENT)
       while((errLen = UDP_RECV(ourSocket, (char *)buffer, (int)bytesToReceive, 0, (RV_SOCKADDR_PTR)sockdata, (SOCKLEN_TYPE *) &socklen))==-1 && errno==EINTR);
#else
...
#define FUNC "RvSocketGetToS6"
...
#if defined(IPV6_TCLASS)
    {
      /* SPIRENT_BEGIN */
#if defined(UPDATED_BY_SPIRENT)
      unsigned int optLen = sizeof(*tos);
#else
...
#define FUNC "RvSocketGetToS4"
    ...
    /* SPIRENT_BEGIN */
#if defined(UPDATED_BY_SPIRENT)
    unsigned int optSize = sizeof(*tos);
#else
    int optSize = (int)sizeof(*tos);
#endif
    /* SPIRENT_END */
#endif
...
#define FUNC "RvSocketSctpBind"
{
    ...
    if((nAddresses > 0) && (sctp_bindx(*sock, (RV_SOCKADDR_PTR)sockdata, nAddresses, (int)operation) != 0)) {
        status = RvSocketErrorCode(RV_ERROR_UNKNOWN);
#if (RV_LOGMASK & RV_LOGLEVEL_ERROR)
        {
            RvChar addr[RV_ADDRESS_MAXSTRINGSIZE];
    ...
    IN  RvLogMgr*           logMgr)
{
    ...
    {
        status = getsockopt(*sock, IPPROTO_SCTP, optName, optVal, &optLen);
    }
    else
    {
