...
static char * SPIRENT_rvTLSGetErrorTxt(
    IN  RvInt           errId);
#endif
/* SPIRENT_END */
...
#if defined(UPDATED_BY_SPIRENT)    
    if(reqClientCert)
       SSL_CTX_set_verify(tlsEngine->ctx, SSL_VERIFY_PEER|SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NULL);
    else
...
#else
        tlsSession->bio = BIO_new_rvsocket(tcpSock,logMgr);/*BIO_NOCLOSE,logMgr*/
#endif
...
#else
	while( ((sslRetCode = SSL_connect(tlsSession->sslSession)) <= 0) && (errno==EINTR));
	if(sslRetCode <= 0) {
#endif
/* SPIRENT_END */
...
#if defined(UPDATED_BY_SPIRENT)
            RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
                             "RvTLSSessionClientHandshake: session (0x%x) general error = %s, errno=%s",
                             tlsSession, SPIRENT_rvTLSGetErrorTxt(sslErrCode),strerror(errno)));
#else
            RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
    ...
    IN  RvLogMgr                *logMgr)
{
    ...
    if (tlsSession->bio == NULL) {
        ...
        tlsSession->bio = BIO_new_socket(tcpSock,BIO_NOCLOSE);
#else
        tlsSession->bio = BIO_new_rvsocket(tcpSock,logMgr);/*logMgr,BIO_NOCLOSE*/
#endif
        tlsSession->sock = tcpSock;
        SSL_set_bio(tlsSession->sslSession,tlsSession->bio,tlsSession->bio);
...
#else
	while( ((sslRetCode = SSL_accept(tlsSession->sslSession)) <= 0) && (errno==EINTR));
	if(sslRetCode<=0) {
#endif
/* SPIRENT_END */
        /* it is supported that most failures are result of
        'will block' condition. In other cases error code
        will be replaced bu the 'general error' */
        stat = RvTLSErrorCode(RV_TLS_ERROR_WILL_BLOCK);
...
#if defined(UPDATED_BY_SPIRENT)
          RvLogInfo(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
                          "RvTLSSessionServerHandshake: session (%p) SSL_ERROR_WANT_WRITE failure",
                          tlsSession));
#else
          RvLogInfo(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
...
#if defined(UPDATED_BY_SPIRENT)
            RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
                             "RvTLSSessionServerHandshake: session (%p) general error = %s, errno =%s",
                             tlsSession, SPIRENT_rvTLSGetErrorTxt(sslErrCode),strerror(errno)));
#else
            RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
...
#else
	while( ((sslRetCode = SSL_read(tlsSession->sslSession,receiveBuf,*receiveBufLen))<0) && (errno==EINTR));
#endif
/* SPIRENT_END */
	if (sslRetCode <= 0)
		rvTLSGetError(tlsSession,sslRetCode,mtx,&sslErrCode);
    ...
    if (tlsSession->renegState == RV_TLS_RENEG_STARTED)
    {
        ...
        if (bWasCompleted == RV_TRUE)
        {
            ...
            RvLogDebug(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
                ...
                tlsSession, logMgr, sslErrCode,
                tlsSession->sslSession->s3->rbuf.left));
        }
    ...
    /* Workaround for case where remote peer, doesn't send application data
       after the last handshake message.
       In this case SSL_read() is blocked on WANT_READ.
       ...
       In order to prevent this deadlock, enable SSL_write, if there is no
       renegotiation now:
          reset tlsSession->requiredForTLSRead. Zero requiredForTLSRead affects
       on rvIsWriteEnabled decision. */
    if (sslErrCode == SSL_ERROR_WANT_READ  &&
        tlsSession->sslSession->s3->rbuf.left == 0)
    {
    ...
    if (tlsSession->sslSession->s3->rbuf.left > 100000)
    {
        RvLogExcep(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
            ...
            tlsSession, logMgr, tlsSession->sslSession->s3->rbuf.left));
    }
    ...
    case SSL_ERROR_WANT_READ:
      RvLogInfo(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
        "RvTLSSessionReceiveBufferEx: session (%p) SSL_RET_WANT_READ failure",
        tlsSession));
      tlsSession->requiredForTLSRead |= RV_SELECT_READ;
      retCode = RvTLSErrorCode(RV_TLS_ERROR_WILL_BLOCK);
      break;
...
#if defined(UPDATED_BY_SPIRENT)
        RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
                        "RvTLSSessionReceiveBuffer: session (%p) general error = %s.errno=%s",
                        tlsSession, SPIRENT_rvTLSGetErrorTxt(sslErrCode),strerror(errno)));
#else
        RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
...
/* SPIRENT_END */
    ...
    /* Ensure that SSL_write is blocked while renegotiating.
       Non-zero requiredForTLSRead causes rvIsWriteEnabled to return FALSE.
       For more details see comments in RvTLSSessionRenegotiate(). */
#if RV_TLS_ENABLE_RENEGOTIATION
    if (tlsSession->renegState == RV_TLS_RENEG_STARTED  &&
        tlsSession->requiredForTLSRead == 0)
...
#else
	while( ((sslRetCode = SSL_write(tlsSession->sslSession,sendBuf,sendBufLen))<=0) && (errno==EINTR));
#endif
/* SPIRENT_END */
    if (sslRetCode <= 0)
        rvTLSGetError(tlsSession,sslRetCode,mtx,&sslErrCode);
...
#if !defined(UPDATED_BY_SPIRENT)
        RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
                         "RvTLSSessionSendBufferEx: session (%p) sent general error = %s, errno=%s",
                         tlsSession, SPIRENT_rvTLSGetErrorTxt(sslErrCode),strerror(errno)));
#else
        RvLogError(RvTLSLogSrc(logMgr), (RvTLSLogSrc(logMgr),
...
#else
	while( ((sslRetCode = SSL_shutdown(tlsSession->sslSession))<=0) && (errno==EINTR));
#endif
/* SPIRENT_END */
