    ...
    OUT RvUint32*       size)
{
    ...
    if (printDate)
        sprintf(dateTxt,"%2.2u/%2.2u/%2.2u ", RvTmGetMon(&tm), RvTmGetMday(&tm), RvTmGetYear(&tm) % 100);
    else
        dateTxt[0] = 0;

    if (printMilliSecs)
        sprintf(millisecsTxt,".%3.3u",((RvUint32)(RvTmGetNsec(&tm)/RV_TIME64_NSECPERMSEC)) % 1000);
    else
        millisecsTxt[0] = 0;
