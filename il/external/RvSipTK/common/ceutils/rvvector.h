...
/* SPIRENT_BEGIN */ \
	void _RvVector##T##InsertMult(RvVector(T)* v, \
	  _RvVectorIter##T it, _RvVectorIter##T first, _RvVectorIter##T last); \
/* SPIRENT_END */ \
...
/* SPIRENT_BEGIN */ \
	void _RvVector##T##InsertMult(RvVector(T)* v, \
		_RvVectorIter##T it, _RvVectorIter##T first, _RvVectorIter##T last) { \
		RvPtrDiffT n = last - first; \
		if(n > 0) \
		{ \
			RvPtrDiffT i = it - rvVectorBegin(v); \
			RvSizeT j; \
			rvAssert(it >= rvVectorBegin(v) && it <= rvVectorEnd(v)); \
			if (rvVectorReserve(T)(v, v->size + n)) { \
				for (j = v->size + n - 1; j >= (RvSizeT)(i + n); --j) { \
					T##ConstructCopy(&v->data[j], &v->data[j - n], v->allocator); \
					T##Destruct(&v->data[j - n]); \
				} \
				while(first != last) \
				{ \
					T##ConstructCopy(&v->data[i], first, v->allocator); \
					++first, ++i; \
				} \
				v->size += n; \
			} \
...
/* SPIRENT_BEGIN */
#define rvVectorInsertMult(T) _RvVector##T##InsertMult
/* SPIRENT_END */
