...
LEGAL_TARGET_OS_VERSION := redhat-6.0 redhat-6.1 redhat-6.2 redhat-7.0 redhat-7.1 redhat-7.2 redhat-7.3 redhat-9 redhat-WS redhat-AS  redhat-ES redhat-Server MontaVista_2.1 MontaVista_3.0 MontaVista_3.1 MontaVista_4.0 MontaVista_5.0 uclinux-2.4 redhat-AS SUSE_8 SUSE_9 SUSE_10
...
ifeq ($(TARGET_OS_VERSION), MontaVista_3.0)
INCLUDE_DIRS += /opt/hardhat/devkit/ppc/7xx/target/usr/local/ssl/include
endif
ifeq ($(TARGET_OS_VERSION), MontaVista_3.1)
INCLUDE_DIRS += /opt/montavista/pro/devkit/ppc/7xx/target/usr/local/ssl/openssl
SSL_DIR := $(shell if ( test -d /opt/montavista/pro/devkit/ppc/7xx/target/usr/local/ssl/lib ) ; then echo /opt/montavista/pro/devkit/ppc/7xx/target/usr/local/ssl/lib ; else echo '' ; fi)
endif

#SSL_DIR := $(shell if ( test -d /opt/hardhat/devkit/ppc/7xx/target/usr/local/ssl/lib ) ; then echo /opt/hardhat/devkit/ppc/7xx/target/usr/local/ssl/lib ; else echo '' ; fi)
#LIB_DIRS += /opt/montavista/pro/devkit/ppc/7xx/target/usr/local/ssl
...
INCLUDE_DIRS += /usr/local/ssl/include /usr/include/kaffe
SSL_DIR := $(shell if ( test -d /usr/local/ssl/lib ) ; then echo /usr/local/ssl/lib ; else echo '' ; fi)
...
ifneq ($(findstring $(LINUX_TYPE),REDHAT SUSE), )
ADAPTERS_$(TARGET_OS) := pmutex posix linux pclock
else
ifneq ($(findstring $(LINUX_TYPE),UCLINUX), )
ADAPTERS_$(TARGET_OS) := pmutex posix utstamp
else
ifneq ($(findstring $(LINUX_TYPE),MVISTA), )
ADAPTERS_$(TARGET_OS) := pmutex posix linux pclock
else
