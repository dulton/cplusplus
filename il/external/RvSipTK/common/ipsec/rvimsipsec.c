...
#define FUNCTION_NAME "rvImsIPSecEstablishSAs"
    ...
    //dprintf("sa[Clnt-Out]:SPI=%d,LclPort=%d,RemPort=%d ",(int)sapdp->iSPI,(int)sapdp->iLclPort,(int)sapdp->iRemPort);
    ...
    //dprintf("sa[Srv-Out]:SPI=%d,LclPort=%d,RemPort=%d ",(int)sapdp->iSPI,(int)sapdp->iLclPort,(int)sapdp->iRemPort);
    ...
    //dprintf("sa[Clnt-In]:SPI=%d,LclPort=%d,RemPort=%d ",(int)sapdp->iSPI,(int)sapdp->iLclPort,(int)sapdp->iRemPort);
    ...
    //dprintf("sa[Srv-In]:SPI=%d,LclPort=%d,RemPort=%d",(int)sapdp->iSPI,(int)sapdp->iLclPort,(int)sapdp->iRemPort);
    ...
    IN RvLogMgr *logMgr)
{
    ...
    RvObjPoolFuncs poolCB = 
            {
                NULL,NULL,spiPoolPageAllocCB,spiPoolPageFreeCB,
                NULL,NULL,NULL,NULL
            };
...
#define FUNCTION_NAME "rvImsConstructCfg"
    ...
    /* SPIRENT_ABACUS_BEGIN */
#if defined(UPDATED_BY_SPIRENT_ABACUS)
    cfg->fairIntPool = createFairIntPool(spiRangeStart,spiRangeFinish);
#endif
    /* SPIRENT_ABACUS_END */
...
#define ITEMS_PER_PAGE 30 /* the sizeof(RvImsSpiStruct) is 24; 24*30 = 720  */
...

    {
        ...
        timeVal = SystemTime.wMilliseconds;
#else
        timeVal = time(NULL);
#endif
    }
    RvRandomGeneratorConstruct(&cfg->iRandObj, timeVal);
...
/* SPIRENT_ABACUS_BEGIN */
#if defined(UPDATED_BY_SPIRENT_ABACUS)
        if(cfg->fairIntPool) deleteFairIntPool(cfg->fairIntPool);
#endif
        /* SPIRENT_ABACUS_END */
...
/* SPIRENT_ABACUS_BEGIN */
  ...
  if(act == SPIGet) {
    return getFromFairIntPool(cfg->fairIntPool, spi);
  }

  if(act == SPIMarkAsBusy) {
    markAsBusyInFairIntPool(cfg->fairIntPool, *spi);
    return RV_OK;
  }

  if(act == SPIReturn) {
    returnToFairIntPool(cfg->fairIntPool, *spi);
    return RV_OK;
  }
#endif
  /* SPIRENT_ABACUS_END */
