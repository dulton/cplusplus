...
/* SPIRENT_BEGIN */
            ...
            if(RvAddressIpv6Construct(ip6anyaddress,RV_ADDRESS_IPV6_ANYPORT, 0, 0, &addr->data.ipv6) != NULL)
                return addr;
#else
...
/* SPIRENT_BEGIN */
#if defined(UPDATED_BY_SPIRENT)
    if(RvAddressIpv6Construct(ip,port, scopeId, flowinfo, &addr->data.ipv6) == NULL)
        return NULL;
#else
...
/* SPIRENT_BEGIN */
#if defined(UPDATED_BY_SPIRENT)
RvUint32 RvAddressIpv6GetFlowinfo(
    IN const RvAddressIpv6 *addr)
{
#if (RV_CHECK_MASK & RV_CHECK_NULL)
    if(addr == NULL)
