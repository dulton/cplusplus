...
MODULES := authenticator call common \
           message message/ims parser regClient pubClient transcClient \
	   stackMgr subscription transaction\
	   transport compartment \
       transmitter mid resolver security secAgree

ifeq ($(SYMBIAN_BASE), )
MODULES := $(MODULES) transport/sctp
endif
...
INCLUDE_FILES_LIST += $(shell find $(PROJECT_PATH) -name RvSip\*.h)
INCLUDE_FILES_LIST += $(PROJECT_PATH)/common/RV_SIP_DEF.h
