...
{
    void *cfg;
    int         (*check_fptr)(int param, char* str, int app_param);
    const char* (*get_fptr)  (int param, char* str, int app_param, int* len);
    int app_param;
} SPIRENT_Encoder_Cfg_Type;


typedef RVAPI RvStatus RVCALLCONV (*SPIRENT_RvSipMsgEncode_Type) (IN  RvSipMsgHandle hSipMsg, IN  HRPOOL hPool, OUT HPAGE* hPage, OUT RvUint32* length);
...
* Counting outgoing messages 
*/
#if defined(UPDATED_BY_SPIRENT)

typedef enum
{
    SPIRENT_SIP_RAW_MESSAGE_INCOMING,
    SPIRENT_SIP_RAW_MESSAGE_OUTGOING
} RvSipRawMessageDirection;

typedef RvStatus (RVCALLCONV *RvSipRawMessageCounter) ();
typedef RvStatus (RVCALLCONV *RvSipRawMessageCounterPerChan) (
                                    IN int cctContext);
typedef struct 
{
    RvSipRawMessageCounter pfnIncoming;
    RvSipRawMessageCounter pfnOutgoing;
    RvSipRawMessageCounterPerChan pfnPerChanIncoming;
    RvSipRawMessageCounterPerChan pfnPerChanOutgoing;
} RvSipRawMessageCounterHandlers;
