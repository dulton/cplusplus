...
ifeq ($(CROSS_COMPILE), on)
RELAX_COMPILER_CHECK := on
endif
...
# UPDATED_BY_SPIRENT
#
# triggers errors in some log parsers
...
endif   # RV_PSOSAPPL_COMPILATION is OFF
...
# Don't install compilation command scripts on host platform
noscriptinstall := on
...
# Don't compile sample applications
nosamples := on

# Don't build test applications
noapp := on

# Build the deprecated core for backward compatibility!
depcore := on

# Build TLS 
tls := off

# Don't build with multithreaded option.
nothreads := off

# Enable IPv6
ipv6 := on

#Builds the modules with support for Poll I/O multiplexing.  
poll := on
