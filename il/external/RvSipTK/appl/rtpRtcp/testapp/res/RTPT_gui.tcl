##############################################################################################
#
#   RTPT_gui.tcl : Prepared gui widgets and gui services
#
##############################################################################################
...
proc histEnt:create { entryBase bindButton varName {tip ""}} {
    ...
    frame $entryBase.histEnt -borderwidth 1 -relief sunk
    ...
    menubutton $entryBase.histEnt.lastNames -indicatoron 1 -menu $entryBase.histEnt.lastNames.01 \
        -relief raised -text "" -padx 7
    ...
    grid $entryBase.histEnt.name -in $entryBase.histEnt -column 0 -row 0 -sticky nswe
    ...
    for {set i 0} {$i <= 5} {incr i} {
        if {[array get app "$entryBase,ip,$i"]!=""} {
            ...
            if {$i == 0} {
                set tmp($varName) $app($entryBase,ip,0)
            }
        }
    }
...
proc test:updateGui {} {
    ...
    # Start looking for PVT leaks if we haven't done just that already
    if {$tmp(updatedGui) == ""} {
    ...
    .test.sess.list delete 0 end
    .test.addrs.inList delete 0 end
    .test.addrs.outList delete 0 end
}
...
# output : none
# return : none
proc test:SetSessions {cur total} {
    global tmp
    set tmp(totalCalls) $total

    .test.status.sess configure -text "Calls: $cur, $total"
    if {$cur == 0} {
