...
RvLogSource* RVCALLCONV   rtpGetSource(RvRtpModule module)
{
    ...
    if (rvRtpInstance.logManager.bLogSourceInited[module]) 
    {
        logPtr = &rvRtpInstance.logManager.logSource[module];
    }
    RvLogLeave(rvLogPtr, (rvLogPtr, "rtpGetSource"));
    return logPtr;
}
...
{
    RvStatus result = RV_ERROR_LIBCODE_RTP;
    RvLogEnter(rvLogPtr, (rvLogPtr, "RvLogSourceDestruct"));
    if (NULL==logMgr)
    {
        RvLogError(rvLogPtr, (rvLogPtr, "RvLogSourceDestruct"));
        RvLogLeave(rvLogPtr, (rvLogPtr, "RvLogSourceDestruct"));
        return result;
    }	
    rvLogPtr = &rvRtpInstance.logManager.logSource[module];
    RvLogLeave(rvLogPtr, (rvLogPtr, "RvLogSourceDestruct"));
    ...
    result = RvLogSourceDestruct(rvLogPtr);
...
{
	RvLogEnter(rvLogPtr, (rvLogPtr, "RvRtpGetLogManager"));
	*logMgrPtr = (RvRtpLogger) logMgr;
	RvLogLeave(rvLogPtr, (rvLogPtr, "RvRtpGetLogManager"));
	return RV_OK;
}
/****************************************************************************
	...
	rvRtpInstance.logManager.printCB = NULL;
    {
        RvInt32 modulesCount = 0;
            for (modulesCount = 0; modulesCount < RVRTP_MODULES_NUMBER; modulesCount++)
                RvRtpSourceConstruct(modulesCount);
    }
/*
...
RvStatus RVCALLCONV  RvRtpDestructLogManager(IN RvRtpLogger* loggerPtr)
{
...

    {
        RvInt32 modulesCount = 0;
        for (modulesCount = 0; modulesCount < RVRTP_MODULES_NUMBER; modulesCount++)
            RvRtpLogSourceDestruct(modulesCount);
    }
		...
		IN RvRtpLoggerFilter filter)
{
	...
	{
	case RVRTP_RTP_MODULE:
		logSourcePtr = &rtpLogManagerPtr->logSource[RVRTP_RTP_MODULE];
		break;
	case RVRTP_RTCP_MODULE:
		logSourcePtr = &rtpLogManagerPtr->logSource[RVRTP_RTCP_MODULE];;
		break;
	case RVRTP_PAYLOAD_MODULE:
		logSourcePtr = &rtpLogManagerPtr->logSource[RVRTP_PAYLOAD_MODULE];;
		break;
	case RVRTP_SECURITY_MODULE:
		logSourcePtr = &rtpLogManagerPtr->logSource[RVRTP_SECURITY_MODULE];;
		break;
    case RVRTP_SRTP_MODULE:
        logSourcePtr = &rtpLogManagerPtr->logSource[RVRTP_SRTP_MODULE];;
        break;
