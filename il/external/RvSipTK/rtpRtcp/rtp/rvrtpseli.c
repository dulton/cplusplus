...
RVAPI RvStatus RVCALLCONV RvRtpSeliInit(void)
{
    ...
    status = RvSelectConstruct(SPIRENT_RTP_ENGINE_SIZE*2, SPIRENT_RTP_ENGINE_SIZE, NULL, &engine);
#else
    ...
    if ((status == RV_OK) && (seliUserFds == NULL) && (seliMaxUserFds > 0) && (seliInitCount == 0))
    {
         status = RvRaConstruct(
             sizeof(RvRtpSeliUserFd), seliMaxUserFds, RV_TRUE, "SELI USER FDs", TMP_LOG, &seliUserFds);
    }
...
RVAPI RvStatus RVCALLCONV RvRtpSeliEnd(void)
{
    ...
    /* SPIRENT_BEGIN */
#if defined(UPDATED_BY_SPIRENT)
    status = RvSelectDestruct(engine, SPIRENT_RTP_ENGINE_SIZE);
#else
