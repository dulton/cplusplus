					    ...
					    RvSize_t  sizeInBits) /* size in bits <= 32 bits */
{
    ...
    *bitOffset+=sizeInBits;
    ...
    while(bBitsInOtherBytes)
    {
        ...
        {
            result <<= sizeInBits;
		    result |= (RvUint32)bitfieldGet(*bufferPtr, -bitsInOtherBytes, sizeInBits);
            bBitsInOtherBytes = RV_FALSE;
        }
	    else
	    {   
            result <<= (8 - offset);
		    result |= bitfieldGet((RvUint32)*bufferPtr, 0, 8 - offset);
					 ...
					 RvUint32  value) 
{
    ...
    *bitOffset+=sizeInBits;
    ...
    while(bBitsInOtherBytes)
    {
        ...
        if (bitsInOtherBytes <= 0)
        {
		    *bufferPtr = (RvUint8)(bitfieldSet(*bufferPtr,value, -bitsInOtherBytes, sizeInBits)&0xff);
            value >>= sizeInBits;
            bBitsInOtherBytes = RV_FALSE;
        }
	    else
	    {   
		    *bufferPtr = (RvUint8)(bitfieldSet(*bufferPtr,value >> bitsInOtherBytes,0, 8 - offset)&0xff);
