...
//SPIRENT_BEGIN
#if defined(UPDATED_BY_SPIRENT)
   INOUT RVAPI RvInt32 RVCALLCONV (*packFunction)(IN void*  hRTP,IN void *buf,IN RvInt32 len,IN  struct RvRtpParam_ *   p, IN void* pfarg);
   INOUT void* pfarg;
   INOUT int singlePeer;
   INOUT     RvUint32    cSrcc;              /* SSRC of received RTP packet */


   IN RvUint32 originalTimeStamp; // The original RTP packet time stamp extracted from PCAP file - RTP Replay feature - 
   IN RvBool   isReplay;          // RV_TRUE:
                                  // - originalTimeStamp will be used for timestamp field in RTP header.
                                  // - sequenceNumber will be used for sequence number field in RTP header.
                                  // - marker will be used for RTP marker bit field in RTP header.
                                  // <note> RvRtpUseSequenceNumber should be called during RTP session creation.
#endif
//SPIRENT_END
