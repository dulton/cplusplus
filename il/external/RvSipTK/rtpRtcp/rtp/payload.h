...
/********************************************************************************
 * RvRtpH261Pack serialises H261 payload header into buffer and sets standard 
...
typedef struct
{
    ...
    RvInt32 dbq;  /* DBQ: 2 bits - the B frame differential quantitization parameter */
    RvInt32 trb;  /* TRB: 3 bits - the B frame temporal reference */
    RvInt32 tr;   /* TR: 8 bits -  Temporal Reference for the P frame */
    RvInt32 gobN; /* GOBN: 5 bits - the GOB number efffective at the start of the packet */
    RvInt32 mbaP; /* MBA: 8 bits - the address within the GOB of the first MB in the packet */
    RvInt32 quant;/* QUANT: 5 bits - the quantitization for the first MB coded at the starting of the packet */
...
/********************************************************************************
 * RvRtpH263Pack serialises H263 payload header into buffer
 * INPUT: buf - buffer to serialize
 *        len - length of the buf
 *        p   - pointer to RvRtpParam ( rtp header)
...
/********************************************************************************
 * RvRtpH263aPack serialises H263a payload header into buffer
 * INPUT: buf - buffer to serialize
 *        len - length of the buf
 *        p   - pointer to RvRtpParam ( rtp header)
 ...
 * bulletlist:
 *   - Contstruct a RvRtpPayloadAMR object using RvRtpPayloadAMRInit.
...
/************************************************************************
 * RvRtpAMRPayloadInit 
 * description:  inits the AMR payload structure
 * parameters:
...
/**********************************************************************************
 * RvRtpH264Construct 
 * description: initialises the H264 payload 
...
} RvRtpPayloadMpegAudio; /* Note Timestamp of MPEG-Audeo is 90000Hz units */
