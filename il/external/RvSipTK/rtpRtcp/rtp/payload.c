...
#define RTP_SOURCE      (localLogSource!=NULL?localLogSource:(localLogSource=rtpGetSource(RVRTP_PAYLOAD_MODULE)))
#define rvLogPtr        (localLogSource!=NULL?localLogSource:(localLogSource=rtpGetSource(RVRTP_PAYLOAD_MODULE)))
		...
		INOUT RvRtpPayloadMpeg4* thisPtr)
{
    ...
    thisPtr->PacketLength = payloadSize;

	if (mimeFormatPtr->AuHeaderSection) 
	{
        thisPtr->AuHeadersLength  = (RvUint16) rvBitfieldRead(bufferPtr, &currentBits, 16);
		...
		{		
			...
			if (mimeFormatPtr->AuSizeSize>0)
                thisPtr->AuHeader[header].AuSize = (RvUint16)
                    rvBitfieldRead(bufferPtr, &currentBits, mimeFormatPtr->AuSizeSize);
            ...
            if (mimeFormatPtr->AuIndexOrAuIndexDeltaSize>0)
			{
				if (header==0)  /* This is AuIndex */
				{
                    ...
                    thisPtr->AuHeader[header].AuIndex = 
                        rvBitfieldRead(bufferPtr, &currentBits, mimeFormatPtr->AuIndexOrAuIndexDeltaSize);
			...
			if (mimeFormatPtr->CTS_deltaSize > 0) /* CTS_flag is present only if CTS_deltaSize >0 */
				thisPtr->AuHeader[header].CTS_flag = 
                    (rvBitfieldRead(bufferPtr, &currentBits, 1) == 0) ? RV_FALSE : RV_TRUE;

            if (mimeFormatPtr->CTS_deltaSize>0 && thisPtr->AuHeader[header].CTS_flag)
                thisPtr->AuHeader[header].CTS_delta =
                    rvBitfieldReadSigned(bufferPtr, &currentBits, mimeFormatPtr->CTS_deltaSize);
            ...
            if (mimeFormatPtr->DTS_deltaSize>0 && thisPtr->AuHeader[header].DTS_flag)
                thisPtr->AuHeader[header].DTS_delta =
                    rvBitfieldReadSigned(bufferPtr, &currentBits, mimeFormatPtr->DTS_deltaSize);
			...
			if (mimeFormatPtr->StreamStateSize>0)
                thisPtr->AuHeader[header].StreamState=
                    rvBitfieldRead(bufferPtr, &currentBits, mimeFormatPtr->StreamStateSize);
			header++;
		}
		thisPtr->AUsNumber = header;
    ...
    if (mimeFormatPtr->AuxiliaryDataSizeSize > 0)
	{
        thisPtr->AuxiliarySection.Size = 
            rvBitfieldRead(bufferPtr, &currentBits, mimeFormatPtr->AuxiliaryDataSizeSize);
        thisPtr->AuxiliarySection.BufferPtr = bufferPtr;
        thisPtr->AuxiliarySection.StartBit = currentBits;
        currentBits+=thisPtr->AuxiliarySection.Size;
        rvBitfieldAdjustToNextByte(&currentBits);
    }

    /* AU Data Section */
	if (mimeFormatPtr->ConstantAuSize > 0)
		thisPtr->AUsNumber = (payloadSize - currentBits/8)/mimeFormatPtr->ConstantAuSize;
	...
	RvRtpPayloadMpeg4* thisPtr)
{
	...
	if (mimeFormatPtr->AuxiliaryDataSizeSize > 0)
	{
        rvBitfieldWrite(bufferPtr, &currentBits,  mimeFormatPtr->AuxiliaryDataSizeSize, thisPtr->AuxiliarySection.Size);
    ...
    thisPtr->PacketLength += currentBits/8;
    bufferPtr+=currentBits/8;
	...
	for (header=0; header < thisPtr->AUsNumber; header++)
	{
        ...
        bufferPtr+=thisPtr->AuHeader[header].DataSize;
        thisPtr->PacketLength += thisPtr->AuHeader[header].DataSize;
	}
         ...
         RvSize_t                   auSize)    /* size of the access unit */ 
{
    ...
    {
		concatinatedPacketLen*=8;
        concatinatedPacketLen+=mimeFormatPtr->AuxiliaryDataSizeSize+thisPtr->AuxiliarySection.Size;
        rvBitfieldAdjustToNextByte(&concatinatedPacketLen);
        concatinatedPacketLen/=8;
    }
	...
	{/*Fragmentation needed*/
		...
		fragmentLength = thisPtr->maxPayloadSize - (thisPtr->PacketLength + concatinatedPacketLen);
    ...
    thisPtr->PacketLength += thisPtr->AuHeader[AuHeaderNumber].DataSize;
		...
		const char *prefix)
{
		 ...
		 thisPtr->mimeFormatPtr->AuxiliaryDataSizeSize>0)
	{
		offset += sprintf(&buffer[offset],"%sAuxiliary Data Size(%2d):		    %d\n",
			prefix, thisPtr->mimeFormatPtr->AuxiliaryDataSizeSize, thisPtr->AuxiliarySection.Size);
		{
		...
		if (thisPtr->mimeFormatPtr->mode & 0x1D)
		{
			...
			if (thisPtr->mimeFormatPtr->AuSizeSize>0 && thisPtr->AuHeader[HeaderNum].AuSize>0)
			{
				if (thisPtr->AuHeader[HeaderNum].DataSize < thisPtr->AuHeader[HeaderNum].AuSize)
					AuSize = thisPtr->AuHeader[HeaderNum].DataSize;
				else
					AuSize = thisPtr->AuHeader[HeaderNum].AuSize;
			}
...
RvStatus RVCALLCONV RvRtpH264Unpack(
									...
									OUT  void *      param)
{
	RvUint8* pBuffer = ((RvUint8*)buf) + p->sByte;
	...
	pPayloadH264->Length = pBuffer -((RvUint8*)buf+ p->sByte);

	RvLogLeave(rvLogPtr, (rvLogPtr, "RvRtpH264Unpack"));
    return  RV_OK;
}
/********************************************************************************
 * RvRtpH264Pack serialises RvRtpPayloadH264 into buffer
 * INPUT: buf - buffer to serialize
 *        len - length of the buf
 *        p   - pointer to RvRtpParam ( rtp header)
 * OUT    param -  pointer to RvRtpPayloadH264
...
RvStatus RVCALLCONV RvRtpH264Pack(
	...
	IN  void     *  param)
{
	RvUint8* pBuffer = ((RvUint8*)buf) + p->sByte; /* space for RTP header */
	RvRtpPayloadH264* pPayloadH264 = (RvRtpPayloadH264*) param;
	RvUint32 Length = 0;
	RvUint32 data32 = 0;
	...
	{
        *pBuffer = (RvUint8) ((pPayloadH264->modes.FU_A.FuHdr.S&0x80) +
			       (pPayloadH264->modes.FU_A.FuHdr.E&0x40) +
			       (pPayloadH264->modes.FU_A.FuHdr.R&0x20) +
				   (pPayloadH264->modes.FU_A.FuHdr.Type&0x1F));
		pBuffer ++;
    ...
    IN	void*        param)
{
    ...
    hPtr[0]  = bitfieldSet2(0,       0                , 11, 5);/* RR */
    hPtr[0]  = bitfieldSet2(hPtr[0], (RvUint16)h263->p, 10, 1);/* P */
    hPtr[0]  = bitfieldSet2(hPtr[0], 0,                  9, 1);/* V */
    hPtr[0]  = bitfieldSet2(hPtr[0], 0,                  3, 6);/* PLEN */
    hPtr[0]  = bitfieldSet2(hPtr[0], 0,                  0, 3);/* PEBIT */
...
RVAPI
RvInt32 RVCALLCONV   myRtpH263PlusUnpack(
    OUT     void*       buf,
    IN	    RvInt32     len,
    OUT     RvRtpParam* p,
    OUT	    void*       param)
...
RVAPI
RvInt32 RVCALLCONV RvRtpH263PlusGetHeaderLength()
{
    RvLogEnter(rvLogPtr, (rvLogPtr, "RvRtpH261GetHeaderLength"));
    RvLogLeave(rvLogPtr, (rvLogPtr, "RvRtpH261GetHeaderLength"));
    return RvRtpGetHeaderLength()+2;
}
#endif
