 ...
 * $Id: //TestCenter/integration/content/traffic/l4l7/il/external/vlc-0.8.6i/modules/misc/network/ipv6.c#1 $
 *
...
#ifdef SPIRENT_CHANGES
    {
        ...
        if( psz_ifname != NULL )
        {
            if( setsockopt( i_handle, SOL_SOCKET, SO_BINDTODEVICE, psz_ifname, strlen( psz_ifname ) + 1 ) < 0 )
                msg_Warn( p_this, "Cannot use %s as output interface: %s", psz_ifname, strerror(errno) );
...
# define IPV6_TCLASS 67         // ref include/linux/in6.h in kernel 2.6 sources
        ...
        if ( i_tclass >= 0 ) 
        {
            if( setsockopt( i_handle, IPPROTO_IPV6, IPV6_TCLASS, &i_tclass, sizeof(i_tclass) ) < 0 )
                msg_Warn( p_this, "Failed to set tclass (%d): %s", i_tclass, strerror(errno) );
        }
    }
#endif
