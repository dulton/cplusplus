 ...
 * $Id: //TestCenter/integration/content/traffic/l4l7/il/external/vlc-0.8.6i/modules/misc/network/ipv4.c#1 $
 *
...
# define strerror( x ) winsock_strerror( strerror_buf )
...

    {
        ...
        if ( i_tos >= 0 ) 
        {
            if( setsockopt( i_handle, IPPROTO_IP, IP_TOS, &i_tos, sizeof(i_tos) ) < 0 )
                msg_Warn( p_this, "Failed to set tos (%d): %s", i_tos, strerror(errno) );
        }
    }
#endif
