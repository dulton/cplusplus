 ...
 * $Id: //TestCenter/integration/content/traffic/l4l7/il/external/vlc-0.8.6i/src/network/udp.c#1 $
 *
...
#ifdef SPIRENT_CHANGES
static int net_SetIfName( vlc_object_t *p_this, int fd, const char *str )
{
    if( setsockopt( fd, SOL_SOCKET, SO_BINDTODEVICE, str, strlen( str ) + 1 ) < 0 )
    {
        msg_Err( p_this, "Cannot use %s as output interface: %s", str, strerror(errno) );
        return VLC_EGENERIC;
    }
...
static int net_SetTos( vlc_object_t *p_this, int fd, int i_tos )
{
    if( setsockopt( fd, IPPROTO_IP, IP_TOS, &i_tos, sizeof(i_tos) ) < 0 )
    {
        msg_Err( p_this, "Failed to set tos (%d): %s", i_tos, strerror(errno) );
        return VLC_EGENERIC;
    }
...
static int net_SetTclass( vlc_object_t *p_this, int fd, int i_tclass )
{
    if( setsockopt( fd, IPPROTO_IPV6, IPV6_TCLASS, &i_tclass, sizeof(i_tclass) ) < 0 )
    {
        msg_Err( p_this, "Failed to set tclass (%d): %s", i_tclass, strerror(errno) );
        return VLC_EGENERIC;
    }
                      ...
                      int i_hlim )
{
    ...
    for( ptr = res; ptr != NULL; ptr = ptr->ai_next )
    {
...
#ifdef SPIRENT_CHANGES
        {
            char *psz_ifname = config_GetPsz( p_this, "ifname" );
            if( psz_ifname != NULL )
            {
                net_SetIfName( p_this, fd, psz_ifname );
                free( psz_ifname );
            }
        }
        ...
        {
            int i_tos = config_GetInt( p_this, "tos" );
            if ( i_tos >= 0 )
                net_SetTos( p_this, fd, i_tos );
        }
        else if (ptr->ai_family == AF_INET6)
        {
            int i_tclass = config_GetInt( p_this, "tclass" );
            if ( i_tclass >= 0 )
                net_SetTclass( p_this, fd, i_tclass );
        }
#endif
