...
#ifndef TIP_CALLBACK_WRAPPER_H
#define TIP_CALLBACK_WRAPPER_H
...
namespace LibTip {
    ...
    /**
     * Callback interface wrapper class.  Tracks a "real" callback
     * pointer and adds a debug print before each function that is
     * invoked.
     */
       
    class CTipCallbackWrapper {
    public:
        /**
         * Only constructor.  Wrapper now owns the callback pointer.
         */
        CTipCallbackWrapper(CTipCallback* callback);

        /**
         * Virtual destructor.
         */
        virtual ~CTipCallbackWrapper();

        /**
         * Set the underlying callback object.  Wrapper now owns the
         * callback.
         */
