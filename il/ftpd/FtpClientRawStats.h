/// @file
/// @brief FTP Client Raw Statistics header file
///
...
#ifndef _FTP_CLIENT_BLOCK_RAW_STATS_H_
#define _FTP_CLIENT_BLOCK_RAW_STATS_H_
...
class FtpClientRawStats : public L4L7_UTILS_NS::StatsTimerManager
{
  public:
    typedef ACE_Recursive_Thread_Mutex lock_t;

    FtpClientRawStats(uint32_t handle)
        : intendedLoad(0),
          activeConnections(0),
          mHandle(handle)
    {
    ...
    void Sync(bool zeroRates = false)
    {
        ...
        if (successfulTransactions)
            avgFileTransferBps = static_cast<uint64_t>(mCumulativeFileTransferBps / successfulTransactions) ;
        else
            avgFileTransferBps = 0 ;        
    }
    ...
    uint64_t mCumulativeFileTransferBps; // for local calculation only -- not in DB.
    uint64_t avgFileTransferBps; 

    uint64_t rxResponseCode150; 
    uint64_t rxResponseCode200; 
    uint64_t rxResponseCode220; 
    uint64_t rxResponseCode226; 
    uint64_t rxResponseCode227; 
    uint64_t rxResponseCode230; 
    uint64_t rxResponseCode331; 
    uint64_t rxResponseCode425; 
    uint64_t rxResponseCode426; 
    uint64_t rxResponseCode452; 
    uint64_t rxResponseCode500; 
    uint64_t rxResponseCode502; 
    uint64_t rxResponseCode530; 
