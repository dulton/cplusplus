/// @file
/// @brief Raw tcp Server implementation
///
...
struct RawTcpServer::IOLogicMessage
{
    enum
    {
        STOP_COMMAND,
...
RawTcpServer::RawTcpServer(uint16_t port, size_t blockIndex, const config_t& config, stats_t& stats, ACE_Reactor *appReactor, ACE_Reactor *ioReactor, ACE_Lock *ioBarrier)
    : mPort(port),
...
{
    mAcceptor->SetAcceptNotificationDelegate(fastdelegate::MakeDelegate(this, &RawTcpServer::HandleAcceptNotification));

    mIOLogicInterface->SetMessageDelegate(fastdelegate::MakeDelegate(this, &RawTcpServer::HandleIOLogicMessage));

    TC_LOG_DEBUG_LOCAL(mPort, LOG_SERVER, "RAWTCP server [" << Name() << "] created");
}
...
bool RawTcpServer::Start(const std::string& ifName, const ACE_INET_Addr& addr)
{
    if (mRunning)
        throw EPHXInternal();
    ...
    {
        TC_LOG_ERR_LOCAL(mPort, LOG_SERVER, "RAWTCP server [" << Name() << "] failed to start on " << addrStr << ": " << strerror(errno));
        return false;
    }
    ...
    TC_LOG_INFO_LOCAL(mPort, LOG_SERVER, "RAWTCP server [" << Name() << "] started on " << addrStr << " (" << ifName << ")");
    return true;
}

///////////////////////////////////////////////////////////////////////////////

void RawTcpServer::Stop(void)
{
    if (!mRunning)
        return;
    
    TC_LOG_INFO_LOCAL(mPort, LOG_SERVER, "RAWTCP server [" << Name() << "] stopping");
    mRunning = false;
...
void RawTcpServer::NotifyInterfaceDisabled(void)
{
    TC_LOG_INFO_LOCAL(mPort, LOG_SERVER, "RAWTCP server [" << Name() << "] disabling protocol");
    Stop();
}

///////////////////////////////////////////////////////////////////////////////

void RawTcpServer::NotifyInterfaceEnabled(void)
{
}

///////////////////////////////////////////////////////////////////////////////

void RawTcpServer::HandleIOLogicMessage(const IOLogicMessage& msg)
{
    switch (msg.type)
    {
      case IOLogicMessage::STOP_COMMAND:
      {
          TC_LOG_INFO_LOCAL(mPort, LOG_CLIENT, "RAWTCP server [" << Name() << "] stopping accepting incoming connections");
      ...
      {
          TC_LOG_INFO_LOCAL(mPort, LOG_CLIENT, "RAWTCP server [" << Name() << "] closing " << mActiveConnCount.value() << " active connections");
...
bool RawTcpServer::HandleAcceptNotification(ServerConnectionHandler& rawConnHandler)
{
    ...
    connHandler->SetCloseNotificationDelegate(fastdelegate::MakeDelegate(this, &RawTcpServer::HandleCloseNotification));
//    connHandler->SetVersion(static_cast<RawTcp_1_port_server::RawTcpVersionOptions>(mConfig.ProtocolProfile.RawTcpVersion));
//    connHandler->SetType(static_cast<RawTcp_1_port_server::RawTcpServerType>(mConfig.ProtocolProfile.RawTcpServerTypes));
    connHandler->SetMaxRequests(mConfig.ProtocolConfig.MaxRequestsPerClient);
    connHandler->SetBodyType(static_cast<RawTcp_1_port_server::BodyContentOptions>(mConfig.ResponseConfig.BodyContentType));
    ...
    const RawTcp_1_port_server::BodySizeOptions bodySizeType = static_cast<RawTcp_1_port_server::BodySizeOptions>(mConfig.ResponseConfig.BodySizeType);
    switch (bodySizeType)
    {
      ...
      default:
          TC_LOG_ERR_LOCAL(mPort, LOG_CLIENT, "Cannot set body size for unknown body size option type (" << tms_enum_to_string(bodySizeType, RawTcp_1_port_server::em_BodySizeOptions) << ")");
          return false;
    }

    const RawTcp_1_port_server::ResponseTimingOptions responseTimingType = static_cast<RawTcp_1_port_server::ResponseTimingOptions>(mConfig.ResponseConfig.L4L7ServerResponseConfig.ResponseTimingType);
    switch (responseTimingType)
    {
      ...
      default:
          TC_LOG_ERR_LOCAL(mPort, LOG_CLIENT, "Cannot set response timing for unknown response timing option type (" << tms_enum_to_string(responseTimingType, RawTcp_1_port_server::em_ResponseTimingOptions) << ")");
          return false;
    }
...
void RawTcpServer::HandleCloseNotification(L4L7_APP_NS::StreamSocketHandler& socket)
{
