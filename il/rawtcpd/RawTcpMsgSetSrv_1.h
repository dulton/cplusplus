...
///
/// $File: //TestCenter/integration/content/traffic/l4l7/il/rawtcpd/RawTcpMsgSetSrv_1.h $
/// $Revision: #1 $
...
#ifndef _RAW_TCP_MSG_SET_SRV_1_H_
#define _RAW_TCP_MSG_SET_SRV_1_H_
...
class RawTcpApplicationProxy;

class RawTcpMsgSetSrv_1 : public MessageSet, public StatsMsgSetSrv_1<&RawTcp_1_port_server::create_req_DoSQLNotify,
                                                                   &RawTcp_1_port_server::create_req_TableEventNotify>
{
  public:
    RawTcpMsgSetSrv_1(RawTcpApplicationProxy& app);
    ~RawTcpMsgSetSrv_1();
  ...
  private:
    ...
    DelegatedMessageHandler *MakeHandler(void (RawTcpMsgSetSrv_1::* handler)(MPS_Handle *handle, uint16_t port, Message *req))
    {
    ...
    /// Methods to handle PHX-RPC messages from RawTcp_1.idl
    void ConfigureRawTcpClients(MPS_Handle *handle, uint16_t port, Message *req);
    void UpdateRawTcpClients(MPS_Handle *handle, uint16_t port, Message *req);
    void DeleteRawTcpClients(MPS_Handle *handle, uint16_t port, Message *req);

    void ConfigureRawTcpServers(MPS_Handle *handle, uint16_t port, Message *req);
    void UpdateRawTcpServers(MPS_Handle *handle, uint16_t port, Message *req);
    void DeleteRawTcpServers(MPS_Handle *handle, uint16_t port, Message *req);
    ...
    typedef StatsMsgSetSrv_1<&RawTcp_1_port_server::create_req_DoSQLNotify, &RawTcp_1_port_server::create_req_TableEventNotify> StatsBase;
    typedef NotifierDB<L4L7_BASE_NS::LoadProfileNotifier, L4L7_BASE_NS::LoadProfileNotifierParams> loadProfileNotifierDB_t;
    
    RawTcpApplicationProxy& mApp;                                 ///< application that owns us
